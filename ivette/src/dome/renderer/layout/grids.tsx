...
const onResizeStop = ({ resizer }) => {
  const shape = makeShapeOfGrid( resizer.grid );
...
class HSPLIT extends React.Component {
  render() {
    const { x, y, width, height, ...handler } = this.props;
    const id = handler.id;
    const { id: sid, offset=0 } = handler.resizer;
    return (
      <DraggableCore
        onStart={() => {
...
export function makeChildrenShape( children, dir )
{
  const makeEltShape = (elt) => {
    const { id, width, height } = elt.props;
    return { kind: 'ITEM', id, width, height };
  };

  const makeBoxShape = (dir, elt) => {
    const { id, children } = elt.props;
    const gs = makeShapes(children);
...
export class GridLayoutCore extends React.Component
{
  ...
  renderSized(width, height)
  {
    ...
    iter(grid, (item) => {
      const e = makeElement(render, item, padding);
