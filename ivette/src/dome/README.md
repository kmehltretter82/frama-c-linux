...
with decently recent versions (`yarn -v 1.5.*` and `node -v 24.*`).
...
The `$(DOME)/template/makefile` is provided with useful targets, but you can also
...
distinct and ignore the entire `$(DOME)` directory in your own versioned files.
