...
function CoverageTable(data: EvaStat.programStatsType): JSX.Element {
...
function Errors(data: EvaStat.programStatsType): JSX.Element {
...
function Alarms(data: EvaStat.programStatsType,
  ...
  const label = (category: EvaStat.alarmCategory): string | undefined => (
    categories.get(category)?.descr
  );

  function order(e1: EvaStat.alarmEntry, e2: EvaStat.alarmEntry): number {
    const { other } = EvaStat.alarmCategory;
    ...
    if (e2.category === other)
      return -1;
    return d || EvaStat.byAlarmCategory(e1.category, e2.category);
  }
...
function Statuses(data: EvaStat.programStatsType): JSX.Element {
  const { assertionsStatuses: assertions, precondsStatuses: preconds } = data;
  const all = (entry: EvaStat.statusesEntry): number => (
...
export function EvaSummary(): JSX.Element {
  const alarmCategories = States.useTags(EvaStat.alarmCategoryTags);
  const data = States.useSyncValue(EvaStat.programStats);
