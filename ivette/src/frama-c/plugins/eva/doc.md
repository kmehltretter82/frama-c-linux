...
All alarms emitted by an Eva analysis are listed in the
[Properties](#framac-properties) component,
...
_main_ function. A click on a call site selects it in the [AST](#framac-ast)
component.
...
for Eva analyses
displayed in the *gutter*, the left-hand column of the AST component.
The gutter can display two kinds of information:
- *status bullets* (colored circles) related to properties; e.g. yellow bullets
  indicating an *unknown* property, meaning Eva was not able to prove it;
- reachability and non-termination, displayed as colored rectangles.
...
**Nonterm** plug-in, whose warnings are visible in the
[Messages](#framac-messages) component.

You can also look for [priority](#eva-alarms) properties in the
[Properties](#framac-properties) component: they indicate that some contexts
lead to *Invalid* statuses, which can cause non-termination.

In the case of non-terminating function calls, you can right-click the
statement and **Go to** the callee, to investigate further, repeating the
...
- open the [Locations](#framac-locations) component, which is now highlighted
in the dock (on the bottom right of the main window).
...
can be inspected in the [Messages](#framac-messages) component.

The _Alarms_ section shows the number of alarms emitted by the Eva analysis.
Alarms can be inspected in the [Properties](#framac-properties) component.
...
Selecting a row selects the related function in the [AST](#framac-ast)
component,
