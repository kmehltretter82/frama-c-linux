...
\subsection{User directories variables}\label{sec:var-user-dirs}

\FramaC provides facilities for sharing information between session using
XDG-like cache, config and state directories. Of course, the default location of
...
\begin{itemize}
  \item Frama-C options: \lstinline{-cache/-config/-state}
  \item Frama-C variables: \lstinline{FRAMAC_CACHE/CONFIG/STATE}
  \item XDG variables: \lstinline{XDG_CACHE/CONFIG/STATE_HOME} (will create subdirectory \lstinline{frama-c} in it)
\end{itemize}

If several are used at the same time for the same directory, the order of
priority is: \FramaC options > \FramaC variables > XDG variables.
...
provide options and variables to override it:
\begin{itemize}
  \item \lstinline{-<plug-in shortname>-cache/config/state}
  \item \lstinline{FRAMAC_<PLUG_IN SHORTNAME>_CACHE/CONFIG/STATE}
\end{itemize}

Again, options have the priority over the variables. Furthermore, plug-in
configuration have the priority over kernel configuration.
