...
may be found in the file \texttt{INSTALL} of the source distribution.
\begin{description}
...
\item[The \caml compiler]\index{OCaml compiler}\footnote{\url{http://ocaml.org}}
  ...
  plug-ins. Version greater or equal than 4.02.3 of the compiler must be used.
...
\item[\tool{OcamlGraph} package:]\codeidx{OcamlGraph} \FramaC needs the
  \tool{OcamlGraph}\footnote{\url{http://ocamlgraph.lri.fr}} package, versions
  $1.8.x$ with $x \geq 5$. This dependency is mandatory for \FramaC.
...
\section{One Framework, Four Executables}\label{sec:modes}

\FramaC installs four executables\footnote{On Windows OS, the
...
\item[\emph{native-compiled \emph{vs} bytecode}:]
  ...
  The bytecode version supports dynamic loading on all architectures,
  and is able to provide better debugging information. Use the native-compiled
  version unless you have a reason to use the bytecode one.
...
\begin{tabular}{ll}
  ...
  \optiondef{-}{print-share-path} & directory of shared resources \\
  \optiondef{-}{print-lib-path} & directory of \FramaC kernel \\
  \optiondef{-}{print-plugin-path} & directories of installed plug-ins \\
  \optiondef{-}{print-config} & a summary of \FramaC configuration \\
...
on the \verb|foo.c| file with the command \verb|frama-c -val foo.c|.
Although the GUI allows to select files and to
launch the value analysis interactively, the
command \verb|frama-c-gui -val foo.c| can be used to launch the value
...
\begin{important}
This last format \textbf{must be used} when \texttt{value} starts with a minus
sign.
\end{important}
...
\begin{important}
This last format \textbf{must be used} if your argument contains a minus sign.
\end{important}
...
\begin{commands}\optionidx{-}{val-use-spec}
\texttt{-val-use-spec="@all, -main"}
\end{commands}
...
\optionuse{-}{val},~\cite{value}) with an unrolling level of 4 (option
\optionuse{-}{ulevel}, Section~\ref{sec:normalize}). Then it re-runs the value
analysis plug-in (option \optionuse{-}{val} is still set) with an unrolling
level of 5.
