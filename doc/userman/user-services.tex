...
interface through the \texttt{Project} menu and through the \texttt{Projects}
sidebar. Among other actions on projects (duplicating, renaming, removing,
saving, \etc), they allow the user to switch between different projects during
the same session.

In the command line, the only way to handle a multi-project session is through
options \optionuse{-}{then-on}, \optionuse{-}{then-last} or
...
might be useful to prevent prohibitive memory consumption. In particular, the
category \texttt{@all\_but\_current} removes all existing projects other than
the current one.
