...
\section*{32.0 (Germanium)}
\begin{itemize}
\item \textbf{Environment Variables:} added section about sandbox mode.
\item \textbf{Preparing the Sources:} added section about Mopsa-DB options
  (\texttt{-mopsa-db}, \texttt{-mopsa-target}, \texttt{-mopsa-list-deps},
  \texttt{-mopsa-exclude-sources}) and restructured the chapter.
\item \textbf{Preparing the Sources:} renamed option
  \texttt{-json-compilation-database} to \texttt{-compilation-db}.
\item \textbf{Variadic Translation:} renamed chapter due to Variadic no
  longer being a plug-in, but a kernel component.
\item \textbf{Customizing Analyzers:} added option
  \texttt{-warn-unaligned-pointer}.
\end{itemize}

\section*{31.0 (Gallium)}
\begin{itemize}
\item  \textbf{Platform-wide Analysis Options:} add section about
  performance-related parameters.
\item \textbf{\FramaC Command Line and General Options:} add section about the
  \texttt{-commands-file} option.
\end{itemize}
...
\begin{itemize}
\item \textbf{Platform-wide Analysis Options:} swap argument order of
...
\item \textbf{Normalizing the Source Code:} document handling of
  new file suffix \texttt{.ci} (Section~\ref{sec:preprocessing-full})
