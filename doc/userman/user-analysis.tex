...
\item[\texttt{-<plug-in shortname>-warn-<stat> <keys>}]
\optionidxdef{-}{<plug-in>-warn-key}
\optionidxdef{-}{kernel-warn-key}
\optionidxdef{-}{<plug-in>-warn-once}
\optionidxdef{-}{kernel-warn-once}
\optionidxdef{-}{<plug-in>-warn-feedback}
\optionidxdef{-}{kernel-warn-feedback}
\optionidxdef{-}{<plug-in>-warn-error}
\optionidxdef{-}{kernel-warn-error}
\optionidxdef{-}{<plug-in>-warn-abort}
\optionidxdef{-}{kernel-warn-abort}
\optionidxdef{-}{<plug-in>-warn-feedback-once}
\optionidxdef{-}{kernel-warn-feedback-once}
\optionidxdef{-}{<plug-in>-warn-err-once}
\optionidxdef{-}{kernel-warn-err-once}
(where \texttt{stat}
is one of \texttt{key}, \texttt{once}, \texttt{feedback}, \texttt{error},
\texttt{abort}, \texttt{feedback-once}, \texttt{err-once})
allows to set the status of a category of warnings. There are eight possible
statuses:
...
\item[err-once] combines the actions of \texttt{error} and \texttt{once}
  statuses.
\end{description}
The table below indicates what will be the new status associated to warning
category \texttt{a} when it is present in \texttt{<keys>}, either in positive
form (\texttt{a} or \texttt{+a}), or in negative form (\texttt{-a}).

\begin{tabular}{|c|c|c|}
\hline
\texttt{stat} & status if positive & status if negative \\
\hline
\texttt{key} & active & inactive \\
\hline
\texttt{once} & once & inactive \\
\hline
\texttt{feedback} & feedback & active \\
\hline
\texttt{error} & error & active \\
\hline
\texttt{abort} & abort & active \\
\hline
\texttt{feedback-once} & feedback-once & once \\
\hline
\texttt{err-once} & err-once & once \\
\hline
\end{tabular}
...
Finally, as for debug categories, passing \texttt{help} in the list of
...
Hence, \texttt{-kernel-warn-abort=*} will stop \FramaC's execution at the first
warning triggered by the kernel.
\end{description}
...
description comes from the Value Analysis manual~\cite{value}. Note that
