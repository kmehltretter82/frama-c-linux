...
\item[feedback-once] a feedback message is emitted only once;
  the status of the category is then reset to \texttt{inactive}.
\item[warning] a proper warning is emitted.
\item[warning-once] a proper warning is emitted only once;
  the status of the category is then reset to \texttt{inactive}.
\item[error] an error is emitted. \FramaC execution continues, but its
  exit status will not be 0 at the end of the run.
\item[error-once] an error is emitted only once;
  the status of the category is then reset to \texttt{inactive}.
\item[abort] an error message is emitted and \FramaC will immediately abort
 its execution.
\end{description}

The \texttt{=<status>} part might be omitted, which is equivalent to asking for \texttt{warning} status.
The new status will be propagated to subcategories, with one exception: statuses
``abort'', ``error'' and ``error-once'' will only be propagated to subcategories
...
\item \optiondef{-}{warn-unaligned-pointer} may be used to check that the code
  does not create pointers whose alignment is not suitable for the type of the
  object that it points to. This option is set by default. In the following
  example, analyzers report by default an error on the second line. Disabling
  the option removes this verification.
\begin{ccode}
char c[5];
int* p = (int*) &c[1];
\end{ccode}
...
The following options allow the user to adjust the amount of memory
used by Frama-C:

\begin{description}

\item \texttt{\optiondef{-}{memory-footprint} <1-10>}
governs the memory usage of Frama-C.
With smaller values, analyses consume much less memory,
but are also slightly slower.
The argument must be between 1 and 10; default is 6.

\item \optiondef{-}{cache-size <1-10>}
controls the amount of memory allocated to some internal caches,
mainly used by the \Eva analysis.
Each increase of 1 doubles the size of the caches, the default value being 2.
Small values are most suitable for the analysis of small programs
(less than 1000 lines), which would not benefit from larger caches.
Higher values (around 7-8) can speed up the analysis of large code bases,
but increase their memory usage.
