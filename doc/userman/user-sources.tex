...
The generation is done according to a \emph{mode} which can be selected via an
option. A plug-in developer has also the possibility to create its own mode
using \FramaC's API. The Plug-in Development Guide~\cite{plugin-dev-guide}
describes all steps required to create and use a custom mode. All available
modes are detailed in the following table:
\begin{table}[h]
...
Additionally, a status will be emitted for each generated clause according to
the following table:
