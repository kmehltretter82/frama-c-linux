...
the default pre-processing command. If patterns \texttt{\%1} and \texttt{\%2}
do not appear in the provided command, the pre-processor is invoked in the
following way.
\begin{commands}
\texttt{<cmd> -o <output file> <input file>}
\end{commands}
...
use the patterns \texttt{\%1} and \texttt{\%2} in the command as place-holders
for the input files and the output file respectively. Here are some examples
...
\texttt{-kernel-warn-abort=-annot-error}\footnote{%
...
\texttt{-kernel-warn-abort=-annot-error}, or even
\texttt{-kernel-warn-key=-annot-error} to silently ignoring any ill-formed
annotation.
...
\item \texttt{\optiondef{-}{inline-calls} <f1,...,fn>} inlines calls to
  functions \texttt{f1,...,fn}. For recursive functions, only the first level
