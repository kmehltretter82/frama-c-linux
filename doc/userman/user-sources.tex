...
as running \texttt{frama-c *.c}. For more complex projects, there are a few
possibilities:

\begin{itemize}
\item If the project is built using \texttt{cc}/\texttt{gcc} (with a Makefile,
  for instance), you can use Mopsa-build\footnote{\textsf{Mopsa} is an
  open-source static analyzer, available at
  \url{https://gitlab.com/mopsa/mopsa-analyzer}. \texttt{mopsa-build} is a tool
  shipped with Mopsa which wraps compiler calls and produces a build
  database that \FramaC can use.} and \texttt{-mopsa-db}
  (detailed in Section~\ref{sec:mopsa}) to automate the parsing process;
\item If you have the list of sources and preprocessing flags
  (included directories and macros, i.e. \texttt{-I}/\texttt{-D} for
  GCC/Clang), you can use option \verb|-cpp-extra-args="-I... -D..."|
  (detailed in Section~\ref{sec:preprocessing-gcc});
\item For more complex cases or if you do not know the above
  commands/options, Section~\ref{sec:preprocessing-full} details the
  entire preprocessing and parsing steps performed by \FramaC.
\end{itemize}

\section{Parsing Using a Mopsa-DB File}\label{sec:mopsa}

The \textsf{Mopsa} analyzer includes the \texttt{mopsa-build} tool, a build
wrapper that produces a build database, similar to tools such as \texttt{bear}
(Build EAR, mentioned in Section~\ref{sec:compilation-db}).

By running \verb|mopsa-build <build commands>|, it produces a
\verb|mopsa.db| file. This binary database can be converted to JSON using
\verb|mopsa-db -json > mopsa-db.json|. This file can then be passed on to
\FramaC using option \optiondef{-}{mopsa-db}. By default, this option
will print the list of {\em targets} identified during the build.
A target is either a library or an executable, that you might want to
analyze with \FramaC.

Adding option \optiondef{-}{mopsa-target} will instruct \FramaC to parse
the source files used to build the given target. For instance, if a
target \texttt{app} has been built using \texttt{main.o} and \texttt{util.o},
which were in turn built using \texttt{main.c} and \texttt{util.c}, run:

\begin{frama-c-commands}
$ frama-c -mopsa-db mopsa-db.json -mopsa-target app
\end{frama-c-commands}

It amounts to running \verb|frama-c main.c util.c| (with relevant
preprocessing flags, if any).

Note that \verb|-mopsa-db| can accept a directory name, in which case it will
search for a file named \verb|mopsa-db.json| inside it. Also, if absent when
another Mopsa-DB-related option is set, it behaves as if the user had added
\verb|-mopsa-db $PWD|, i.e. set it to the current directory.

Other Mopsa-DB-related options are available, but they are only occasionally
useful:

\begin{description}
\item \texttt{\optiondef{-}{mopsa-exclude-sources} <sources...>} tells \FramaC
  not to parse such sources, even if they are present in the Mopsa-DB file as
  dependencies of a selected target.
\item \texttt{\optiondef{-}{mopsa-list-deps} <targets>} prints the list of
  sources that were used to produce the given targets, along with their
  preprocessing flags, then exits.
\item \optiondef{-}{permissive} allows \FramaC to proceed parsing even when
  there are missing sources, instead of aborting. Used when the Mopsa-DB
  contains spurious entries.
\end{description}

Note that, even when using a Mopsa-DB, you can add other source files (e.g.
additional stubs) to the command line, to be parsed along with those from
the Mopsa-DB.

\section{Preprocessing Sources in a GCC-Compatible Environment}
\label{sec:preprocessing-gcc}

This section assumes you cannot or do not want to use \texttt{-mopsa-db}
directly.
...
\subsection{Preprocessing Using a Compilation Database}\label{sec:compilation-db}
...
it to retrieve the above preprocessing flags for each file in the database,
via option \optiondef{-}{compilation-db} \texttt{<path>},
where \texttt{<path>} is the path to the JSON file or to a directory containing
a file named \texttt{compile\_commands.json}.

JSON compilation databases can be produced by \textsf{CMake} (with option
\verb|-DCMAKE_EXPORT_COMPILE_COMMANDS=1|) or build wrappers such as
Build~EAR\footnote{\url{https://github.com/rizsotto/Bear}}.

Note that, unlike a Mopsa-DB, a \verb|compile_commands.json| file does not
include linking information and cannot track dependencies from binaries to
sources. This means you need to provide the list of source files to be parsed.

With \verb|compilation-db| set, \FramaC will parse
...
\section{Preprocessing Sources, Full Details}\label{sec:preprocessing-full}

This section contains details about how preprocessing and parsing are done
in \FramaC, which should help deal with non-standard usage scenarios.
...
Note that some plug-ins and kernel modules, such as \textsf{Variadic}
(described in chapter~\ref{user-variadic}), perform further AST
transformations before most analyses are run.

By default, \FramaC considers that \gcc is installed and uses it as
preprocessor.
If option \texttt{-cpp-extra-args} mentioned in the previous section is not
enough for your needs (e.g. you use a non-GCC-compatible compiler), you can
replace the preprocessing command
...
Here are some examples using this option.
\begin{frama-c-commands}
...
\subsection*{Preprocessing ACSL Annotations}

By default, \acsl annotations are preprocessed (option \optiondef{-}{pp-annot}
is set by default).
...
users can apply option
\texttt{-kernel-warn-key annot-error=warning}\footnote{%
See section~\ref{sec:feedback-options} for more information on warning statuses.}
to continue parsing after emitting a warning when an \acsl annotation fails to type-check.
...
Default is \texttt{default}.
This includes:
\begin{itemize}
\item \texttt{default}: chose between \texttt{gcc} or \texttt{msvc} depending on
the machdep (see \texttt{-machdep}).
...
\item \texttt{gcc-short-enums}: same behavior than \texttt{gcc-enums} but
starting from \texttt{char} instead (\gcc's \texttt{-fshortenums} option)
\item \texttt{msvc}: same behavior than \texttt{int}
\end{itemize}
...
this format in more detail, as well as the use of \texttt{frama-c-script make-machdep}
to automatically generate it. The YAML schema contains a list of builtin
...
(using \texttt{-cpp-extra-args} or its per-file counterpart as in
Sect.~\ref{sec:preprocessing-gcc},
or a compilation database as in Sect.~\ref{sec:compilation-db}). In addition,
...
\item \textttdef{\_\_FC\_FORCE\_POSIX\_C\_SOURCE}: By default, \FramaC's positions
  \lstinline|_POSIX_C_SOURCE| according to the relevant machdep field
  \lstinline|posix_c_source|. Defining
  \lstinline|__FC_FORCE_POSIX_C_SOURCE| overrides this value. If the macro is
  defined to an empty value, \lstinline|_POSIX_C_SOURCE| will be undefined.
\end{description}
...
adds \verb+-I$(frama-c -print-share-path)/libc+ to the preprocessor
...
(see section~\ref{sec:preprocessing-full}).
