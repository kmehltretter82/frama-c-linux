...
\texttt{mode} is used to generate missing specifications. \texttt{mode} can
be either ``frama-c'', ``acsl'', ``safe'' or any custom mode registered in
a plug-in.
...
and is replaced by \texttt{Frama\_C}. For example:
\begin{frama-c-commands}
  \$ frama-c -generated-spec-custom requires:safe
\end{frama-c-commands}
Here every clause is generated using the default mode \texttt{Frama\_C}
except for \texttt{requires} which is generated using the mode
\texttt{Safe}.
\end{description}
