...
\section{Normalizing Contacts}\label{sec:normalize-contracts}
...
some specification to improve their performances. When they do, then can ask
\FramaC's kernel to generate specifications.

For example, let's say we have a function \texttt{f} that is only declared and
has no \acsl \texttt{assigns} clause, but these \texttt{assigns} are required by
\Eva. \Eva will then ask the kernel to generate \texttt{assigns} before resuming
...
developer has also the possibility to create its own mode using \FramaC's API.
...
\texttt{mode} will be used to generate missing specifications. \texttt{mode} can
be either ``frama-c'', ``acsl'', ``safe'' or any custom mode registered inside
a plug-in.

\item \texttt{\optiondef{-}{generated-spec-custom
<clause\_1:mode\_1,clause\_2:mode\_2,...>}} has a similar behavior than
\optionuse{-}generated-spec-mode, except that it allows the user to choose a
\texttt{mode} individually for each clause. It also allows the user to ignore
\FramaC and/or plug-ins needs by skipping the generation for a clause, with the
mode ``skip''. This mode is not available with \optionuse{-}generated-spec-mode
and will be replaced by \texttt{Frama\_C}. For example:
\begin{frama-c-commands}
  \$ frama-c -generated-spec-custom requires:safe
\end{frama-c-commands}
Here every clause will be generated using the default mode \texttt{Frama\_C}
except for \texttt{requires} which will be generated using the mode
\texttt{Safe}.
\end{description}

As mentionned, the generation can also be done by combining existing clauses
from other behaviors following this algorithm:
\begin{itemize}
  \item[1.] If the function contract has some clauses \texttt{C} in unguarded
  (without \texttt{assumes} clauses) behaviors, we combine them inside the
  default behavior.
  \item[2.] Else, if it has some clauses \texttt{C} in complete behaviors, we
  combine them inside the default behavior.
  \item[3.] Else, if it has some clauses \texttt{C} anywhere, we combine them inside
...
last point of our algorithm to use the selected mode.}\\

Additionally, a status is emitted for each generated clause. A summary can
be found in table~\ref{table:populate-status}.
