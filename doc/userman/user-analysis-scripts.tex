...
\item[main function]: the function where the analysis will start; it is often
  \texttt{main}, but not always. ({\em Note: \FramaC itself does not require
...
in section~\ref{sec:preprocessing-gcc}). This leads to a different workflow:
...
\item \texttt{fcmake <target>.eva.ivette}: open the results in Ivette
\end{itemize}
...
\item[<target>.eva.ivette]: loads the result of the corresponding
  \texttt{<target>.eva} session and opens it in Ivette. This allows inspecting
  the results of \Value. This target always opens Ivette, even when no
...
or in the GUI. For instance, the \texttt{.ivette} predefined target works out
of the box in this case: running \texttt{fcmake target.wp.ivette} will load
the saved session in Ivette.
...
\begin{description}
\item[configure]: runs a \texttt{configure}
  ...
  parsing the sources. Provided on a best-effort basis, but does work for all
  scripts.
