...
\item The user identifies a C code base on which they would like to run
  Frama-C;
\item The user cofigures the initial analysis
...
\item copy the {\em analysis template}, a Makefile located in
  ...
  This Makefile is usually renamed as \texttt{GNUmakefile} inside
  \texttt{.frama-c}.
\item open the \texttt{GNUmakefile} and fill in the required information:
  architecture, list of sources, preprocessing flags (both {\em include}
  directories for headers and {\em macro definitions}).
\end{enumerate}
