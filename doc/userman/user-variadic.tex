%% --------------------------------------------------------------------------
%% --- Variadic Translation (kernel component)
%% --------------------------------------------------------------------------

\chapter{Variadic Translation}
\label{user-variadic}

This chapter briefly presents the transformations performed by kernel
option \optiondef{-}{variadic-translation} (enabled by default), which
translates calls to variadic functions into calls to
...
challenges to code analysis. The Variadic Translation component helps
dealing with some
...
arguments have no name and no statically known type.
Option \optionuse{-}{variadic-translation}
performs a semantically-preserving translation of calls to such
...
Since \optionuse{-}{variadic-translation} is enabled by default, running
\FramaC on this code will activate the variadic translation.
The main differences in the translated code are:
...
and modified arguments (namely for \verb+scanf+).
\optionuse{-}{variadic-translation}
 generates such specifications whenever possible.
...
Here is a concrete example of a program using a variadic function from the C
standard library:

\cinput{./examples/variadic.c}

And here is the output produced by \optionuse{-}{variadic-translation}:

\cinput[linerange=2-]{./examples/generated/variadic.c}

\subsection{Main options}

By default, \optionuse{-}{variadic-translation} is enabled and runs after parsing.
A few options are available to modify this behavior:

\begin{description}

\item \texttt{\optiondef{-}{no-variadic-translation}} : disables the translation;
  to be used in case it interferes with some plug-in or analysis.
\item \texttt{\optiondef{-}{no-variadic-strict}} : disables warnings about
...
Some of the issues detected by \optionuse{-}{variadic-translation}, namely some kinds of
...
\optionuse{-}{variadic-translation}, not all of them immediately obvious.
...
The warning emitted in this case will mention a
...
The output produced by \optionuse{-}{variadic-translation}, in particular when using
