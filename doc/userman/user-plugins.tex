...
\optiondef{-}{no-autoload-plugins}. Then the plugin to load can be selected using \texttt{\optiondef{-}{load-plugin} <plugin-name>} (e.g. \texttt{aorai}). Since \FramaC plugins are also OCaml libraries it is possible to use \texttt{\optiondef{-}{load-library} <library-name>} (e.g. \texttt{frama-c-aorai}). Both options can accept comma-separated list of names.
