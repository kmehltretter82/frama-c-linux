...
Running \textttuse{frama-c-gui} displays the
...
For an Opam-based installation of \FramaC, running \textttuse{frama-c-gui} for
the first time will trigger a script that downloads, compiles, installs and runs
Ivette. This script requires some tools such as Node.js and Yarn.
Subsequent executions of \textttuse{frama-c-gui} will directly run the installed
executable.
...
\textttuse{frama-c-gui} accepts almost all command-line options that
\texttt{frama-c} does, so you can usually replace \texttt{frama-c <options>}
with \texttt{frama-c-gui <options>} to run an analysis and see its results.
