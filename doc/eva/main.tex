...
The option \texttt{-eva-taint-secureflow} enables a secure-flow analysis based
on the taint domain (as such, it requires the taint domain to be active). This
analysis detects violations of the non-interference property---that is, the
absence of unauthorized information flows between security levels.
Non-interference requires that changes to high-security (e.g., \texttt{private})
data do not influence low-security (e.g., \texttt{public}) data, whether through
explicit data dependencies or implicit control dependencies. In other words, a
program satisfies non-interference if an observer with access only to public
information cannot deduce anything about private information from its behavior.

Data may be assigned a security level---either \lstinline|private| or
\lstinline|public|)---by using the \lstinline|__attribute__((...))| type
annotation. A current limitation restricts these annotations to aggregate types
as a whole; they cannot be specified for individual members or elements within
an aggregate.
...
\begin{frama-c-commands}
frama-c -eva -eva-domains taint -eva-taint-secureflow \
        -eva-warn-key=acsl:unsupported=inactive sf.c
\end{frama-c-commands}
