...
% Commands to put references in the index:
...
values taken by the loop counter in the \emph{Eva Values} view.
In this case, the loop condition is \verb|while(i < (size_t)WCNT)|,
where \verb|WCNT| has the constant value 4 and values of variable \verb|i|
range from 0 to 4, so the loop bound is 4.
...
\begin{frama-c-commands}
frama-c-gui -eva-precision 3 -eva *.c
\end{frama-c-commands}

The GUI allows the user to navigate the source code, to inspect the sets of
values inferred by the analysis and to get
a feeling of how it works.

Note that the GUI has several different uses, each of them with a customizable
{\em View} that encapsulates the most relevant components. The {\em Eva Values}
view is suitable for inspecting values. When selected, is displays the code
seen by Frama-C in the the upper left component (AST).

Right-clicking on the function name
...
Other views often useful for \Eva{} are {\em Eva Summary} and
{\em Properties}. Further details about the GUI are presented in
chapter~\ref{gui}, and also in the online help: click on the ``?''
(question mark) button in the top right corner of a component to
...
The Properties component is useful for inspecting alarms. In this component:

\begin{itemize}
\item an Invalid status indicates that the
  analysis is {\em certain} that a problem has arrived;
...
\item the cases marked with an exclamation mark in the {\em Priority}
  column\footnote{You can right-click the table header to add/remove columns,
  such as Priority.} indicate situations
  which are likely to correspond to definitive problems, and thus should be
  treated before others\footnote{The {\em exact} meaning of
  ''high-priority alarms'' is
  too complex to be presented here; as a first approximation, these can be
  seen as intermediate between red and yellow alarms.};
\item the remaining cases are usually of {\em Unknown} status.
\end{itemize}

In the case of our remaining alarm, it is yellow, but it appears in the
Priority column. This indicates it is more likely to correspond to a real
issue.

Extra information can be observed by clicking the table row containing the
alarm, which highlights it in the source code, and then selecting expression
\lstinline|hash| in line 17 and reading the contents of \textsf{Eva Values}:
\begin{logsmath}
  [0] $\in$ {200}
...
described in section \ref{builtins_observation}) can help to iterate towards
...
Note that line 6 of this example could also be considered as
problematic in the strictest interpretation of the standard.
By default, \Eva{} does not emit alarm for this line,
but the \lstinline|-warn-invalid-pointer| parameter enables
alarms on such invalid pointer arithmetic.

\subsubsection{Unaligned pointer}

On each pointer of the analyzed program, \Eva{} checks that all the addresses
it may point to are properly aligned according to the pointer type.
The alignment of each C type depends on the target platform,
defined through the \lstinline|-machdep| parameter. \lstinline|_Alignas|
specifiers are properly taken into account.

Whenever \Eva{} is unable to verify the alignment of a pointer, it emits an
alarm.
Such alarm uses the \acsl{} predicate \lstinline|\aligned(p, alignof(type))|
to express that pointer \lstinline|p| must be aligned according to its
\lstinline|type|, meaning that the address pointed to by \lstinline|p| must be
a multiple of the alignment of \lstinline|type| in the target platform.
The \lstinline|\aligned| predicate can also be used in standard \acsl{}
assertions to verify more properties on pointer alignment.

\csource{examples/alarms/pointer_alignment.c}

In the above example, the analysis emits alignment alarms on the conversion of
\lstinline|c| and \lstinline|c2| as a pointer to \lstinline|int|.
On the other hand, the next two conversions are valid and no alarm is emitted.

\begin{logsmath}
[eva:alarm] pointer_alignment.c:5: Warning:
  unaligned pointer creation. assert \aligned((char *)c,alignof(int));
[eva:alarm] pointer_alignment.c:6: Warning:
  unaligned pointer creation. assert \aligned((char *)c2,alignof(int));
\end{logsmath}

All alignment alarms can be disabled with the
\lstinline|-no-warn-unaligned-pointer| parameter.
...
When using \lstinline|frama-c-gui| (the graphical interface), all
messages are shown in the dedicated \emph{Messages} component.
Section \ref{verbosity} explains how to configure \Eva{} verbosity to hide or
show such messages.
...
The \Eva{} public API is available in the file \texttt{plugins/eva/src/Eva.ml}.
...
\ref{functions} is not the function \lstinline|main_1| but the following
...
in sections \ref{boucles} and \ref{functions}: the set of even numbers
...
\subsection{Controlling the verbosity}
\label{verbosity}

The \texttt{-eva-verbose} parameter controls the verbosity of the analysis,
from 0 (no message is printed) to 11 (all possible messages are printed).
The default verbosity is 5, which is best suited for understanding the analysis
results on small examples, but is usually too verbose on larger case studies.

Most \Eva{} messages are associated with a message category (or message key),
which appears as prefix of the message. For instance, the analysis summary
starts with \texttt{[eva:summary]}, as it is emitted by \Eva{} with the
\texttt{summary} category.
\texttt{-eva-verbose N} automatically sets up message categories according to
the specified level \texttt{N}. The \texttt{-eva-msg-key} parameter can also be
used to enable or disable some message categories, and takes precedence over
\texttt{-eva-verbose}, allowing for a finer verbosity configuration.
See \texttt{-eva-msg-key help} for details on \Eva{} message categories,
and the Frama-C User Manual for explanations on message categories in general.

Finally, \texttt{-eva-verbose} only affects result and feedback messages, but
not warnings or errors.

In particular, \Eva{} emits all the alarms it finds (see section
\ref{obligations}) as both ACSL assertions and textual warnings in the
log. Since ACSL assertions are stored by the Frama-C kernel, and can be
reviewed in the graphical interface or output by plugins such as Report,
the textual warnings are partly redundant.
Warning category\footnote{See Frama-C User Manual~\cite{FCUserMan}.}
...
\subsection{Partitioning on conditional structures}
\label{partitioning-conditional-structures}
...
Locally, annotation \lstinline|//@  split \cases;| can be used to separate the
analysis for each branch of the next \lstinline|if-then-else| or
\lstinline|switch| statement, even after their blocks are closed.
By default, such case-base reasoning are maintained until the end of the current
function, or indefinitely with \lstinline|-eva-interprocedural-splits|.
To avoid slowing down the analysis, one may also cancel any active
\lstinline|split \cases| with the symmetric annotation \lstinline|merge \cases|.
...
\item[Dynamic split] with annotation \lstinline|//@  dynamic_split e;| \\
  ...
  expression always evaluates to a singleton in each analysis state, until a
...
\item While the number of simultaneous splits (whether local with annotations
  ...
  is controlled across the whole program.
...
in Section~\ref{builtins_observation}, and the log category of each domain
...
partition of {\it segments}. A segment is a set of array cells with consecutive
indexes. A classical segmentation would be the division of an array into three
segments around a program variable {\tt i}: the set of cells whose index is
...
will work on most simple loops, but may fail outside the most common patterns.
...
Option \texttt{-eva-domains taint} performs a taint analysis, a form of
information-flow analysis, that tracks how data originating from untrusted or
sensitive sources---such as external inputs or secrets---propagate through the
program and potentially influence its execution.
...
The specified set of lvalues becomes tainted just after the code annotation, or
at the return of the function with the \texttt{taints} clause. The taint is then
propagated during the \Eva{} analysis, which computes an over-approximation of
the set of tainted locations at each program point.
...
\subsubsection{Direct and indirect taints}
Taint analysis distinguishes between direct and indirect taints. Accordingly, a
memory location may be \emph{directly}, \emph{indirectly}, or both directly and
indirectly tainted.

Direct taints account for \emph{data dependencies} among memory locations: a
first location is said to be \emph{data-dependent} on a second location if the
value of the former is computed from the value of the latter.
In practice, direct taints are introduced by means of \acsl{} annotations
\lstinline|//@taint <lvalues>;| or clauses \lstinline|taints <lvalues>;|, and
they are propagated through assignments \lstinline|lvalue = expr|: the location
\lstinline|lvalue| becomes directly tainted whenever a location read in
\lstinline|expr| is directly tainted.

Indirect taints account for \emph{control dependencies} among memory locations:
a first location is said to be \emph{control-dependent} on a second location if
the value of the former may depend on a control-flow decision or an address
computation based on the value of the latter. In practice, they are introduced
and propagated through assignments of the form \lstinline|lvalue = expr|: the
location \lstinline|lvalue| becomes indirectly tainted whenever one of the
following applies:
\begin{enumerate}
  \item \lstinline|lvalue = expr| occurs in the scope of a conditional
  statement (possibly through function calls) whose controlling expression is
  tainted;
  \item a location read in \lstinline|expr| is indirectly tainted;
  \item the address computation of \lstinline|lvalue| depends on the value of
  a tainted location;
  \item the address computation of a location read to compute the value of
  \lstinline|expr| depends on the value of a tainted location.
\end{enumerate}

To illustrate this distinction, let us consider the following example:
\begin{listing-nonumber}
void fun(int t) {
  //@ taint t;                // 't' becomes directly tainted.

  int x, y, z, w = 0;

  x = t + 1;                  // 'x' becomes directly tainted via 't'.

  // CASE 1.: 'z' and 'w' become indirectly tainted as their value depend on the
  // value of the controlling expression 'x', which is tainted.
  // Moreover, 'z' becomes also directly tainted as it may take the value of
  // 't' in the else-branch. 
  if (x) {
    z = 1;
    w = 0;
  }
  else {
    z = t;                    // 'z' becomes directly tainted via 't'.
    w = 1;
  }

  // CASE 2.: 'y' becomes indirectly tainted via 'w'.
  y = w;

  int buf[2] = {0};

  // CASE 3.: 'buf[w]' becomes indirectly tainted as its address depends on
  // the value of the tainted location 'w'.
  buf[w] = buf[1] + 1;

  // CASE 4.: 'buf[0]' becomes indirectly tainted as the address of 'buf[w]'
  // depends on the value of the tainted location 'w'.
  buf[0] = buf[w] - 1;
}
\end{listing-nonumber}

To reflect this distinction, the predicates \lstinline|\tainted_directly| and
\lstinline|\tainted_indirectly| refine the general \lstinline|\tainted|
predicate: \lstinline|\tainted_directly| (resp. \lstinline|\tainted_indirectly|)
can be used to verify that a value is \emph{not} directly (resp. indirectly)
tainted.
...
\begin{listing-nonumber}
  void fun(int a, int b, int t) {
    //@ taint t;
    int x = a + t;
    //@ check !\tainted_indirectly(x);  // TRUE
    //@ check \tainted_directly(x);     // UNKNOWN
    //@ check \tainted(x);              // UNKNOWN

    int y = 0;
    if (t)
      y = a - b;
    //@ check !\tainted_directly(y);    // TRUE
    //@ check \tainted_indirectly(y);   // UNKNOWN
    //@ check \tainted(y);              // UNKNOWN
  }
\end{listing-nonumber}

As with \lstinline|\tainted|, the predicates \lstinline|\tainted_directly| and
\lstinline|\tainted_indirectly| can be used in \acsl{} assertions to refine the
over-approximation of tainted locations, with respect to direct and indirect
taints respectively.

\subsubsection{Secure-flow analysis}
The option \texttt{-eva-secure-flow} enables a secure-flow analysis based on the
taint domain (this option automatically activates the taint domain).

This analysis aims at verifying non-interference properties---that is, the
absence of unauthorized information flows between security levels---and detects
when they are violated. Non-interference requires that changes to high-security
\texttt{private} data do not influence low-security \texttt{public} data,
whether through explicit data dependencies or implicit control dependencies. In
other words, a program satisfies non-interference if an observer with access
only to public information cannot deduce anything about private information from
its behavior.

To instruct the analysis, data may be assigned a security level---either
\lstinline|private| or \lstinline|public|---by using \lstinline|__attribute__|
type annotation. A current limitation restricts these annotations to aggregate
types as a whole; they cannot be specified for individual members or elements
within an aggregate.

\begin{listing-nonumber}
  int __attribute__((private)) secret;
  int __attribute__((public))  nonsecret;
\end{listing-nonumber}

Non-interference violations are then reported as warnings during the \Eva{}
analysis. A violation occurs whenever the over-approximation of the set of
private locations intersects that of public locations---either directly or
indirectly---or when it influences the program's control flow.

A custom \acsl{} annotation can be used to verify the security status of some
location.

\begin{listing-nonumber}
  //@ assert security_status(secret) == private;    // UNKNOWN
  //@ assert security_status(nonsecret) == public;  // TRUE
\end{listing-nonumber}

Since secure-flow analysis makes use of the taint domain, it only infers
over-approximations of the private locations: while locations in the
over-approximation \emph{may be} private, all other locations are proven to be
public.
...
\begin{frama-c-commands}
frama-c -eva -eva-secure-flow sf.c
\end{frama-c-commands}

\Eva{} emits the following warnings:
\begin{logsmath}
[eva:secure-flow:condition] sf.c:8: Warning:
  non-interference violation on condition involving {user_input; secret}
[eva:secure-flow:indirect] sf.c:9: Warning:
  indirect non-interference violation on {y}
[eva:secure-flow:indirect] sf.c:12: Warning:
  indirect non-interference violation on {z}
[eva:alarm] sf.c:17: Warning: assertion got status unknown.
[eva:alarm] sf.c:18: Warning: assertion got status unknown.
[eva:secure-flow:indirect] sf.c:23: Warning:
  indirect non-interference violation on {y; z}
[eva:alarm] sf.c:24: Warning: assertion got status unknown.
[eva:alarm] sf.c:25: Warning: assertion got status unknown.
\end{logsmath}

Warnings classified as \lstinline|eva:secure-flow| are non-interference
violations. The first warning indicates a control-flow dependency on private
data, as the branch taken depends on the value of \lstinline|secret|. The
subsequent two warnings report indirect violations since variables \lstinline|y|
and \lstinline|z|, which are declared public, are assigned within branches
controlled by private information. Similarly, the last warning occurs because
the assignment \lstinline|*p = 1| may write to either \lstinline|y| or
\lstinline|z| through the pointer \lstinline|p|, which has become private via
private-dependent control flow.

The assertion warnings reflect the over-approximation inherent in the analysis.
The assertion on the security status of \lstinline|x| is proven valid because
\lstinline|x| is assigned independently of any private data. In contrast, since
\lstinline|y| and \lstinline|z| are in the \emph{over-approximation} of private
locations, the analysis cannot definitively prove they are private---they may or
may not be private due to the lack of precision in the analysis.
...
The numerors domain only infers error values for scalar variables. Structure
...
\begin{center}
  \begin{tabular*}{1\textwidth}{@{\extracolsep{\fill}}>{\raggedright}m{5cm}>{\raggedright}m{9cm}c}
    Syntax & Description & Ref
  \tabularnewline
  \midrule
  \midrule
    \lstinline|//@  loop unroll <N>;|
  ...
  & \tabularnewline
  \midrule
    \lstinline|//@  split \cases;|
  & Case-based reasoning: separate the analysis for each branch of the next
    \lstinline|if-then-else| or \lstinline|switch| statement.
  & \multirow{2}{*}{\ref{partitioning-conditional-structures}}
  \tabularnewline
  \cmidrule{1-2} \cmidrule{2-2}
    \lstinline|//@  merge \cases;|
  & Cancel any active \lstinline|split \cases|.
  & \tabularnewline
  \midrule
    \lstinline|//@  slevel <N>;|
