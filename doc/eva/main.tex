...
Taint analysis accounts for both data and control dependencies: a value may
become tainted either directly, through assignments that use tainted data, or
indirectly, through control flow and computations that depends on tainted
values. To reflect this distinction, the predicates
\lstinline|\tainted_directly| and \lstinline|\tainted_indirectly| refine the
general \lstinline|\tainted| predicate: \lstinline|\tainted_directly| (resp.
\lstinline|\tainted_indirectly|) can be used to verify that a value is
\emph{not} directly (resp. indirectly) tainted.
