...
become tainted either directly, through assignments and computations that use
tainted data, or indirectly, through control flow that depends on tainted
values. To reflect this distinction, the predicates
\lstinline|\tainted_directly| and \lstinline|\tainted_indirectly| refine the
general \lstinline|\tainted| predicate. In the same spirit as
\lstinline|\tainted|, \lstinline|\tainted_directly| can be used to verify that a
value is \emph{not} directly tainted, while \lstinline|\tainted_indirectly| to
verify that a value is \emph{not} indirectly tainted.
