...
location may be \emph{directly}, \emph{indirectly}, or both directly and
indirectly tainted.

Direct taints account for \emph{data dependencies} among program locations: a
...
\lstinline|//@taint <lvalues>;| or clauses \lstinline|taints <lvalues>;|. They
are propagated through assignments \lstinline|lvalue = expr| whenever
a location occurring in \lstinline|expr| is directly tainted or data-dependent
on another location.

Indirect taints account for \emph{control dependencies} among program locations:
...
and propagated through assignments of the form \lstinline|lvalue = expr|
whenever one of the following applies:
\begin{enumerate}
  ...
  tainted;
  \item the value of a location occurring in \lstinline|expr| depends on the
  value of an indirectly tainted location;
...
void fun(int t) {
  ...
  // CASE 3.: 'buf[w]' becomes indirectly tainted as the address 'buf + w'
  // depends on the value of the tainted location 'w'.
  buf[w] = buf[1] + 1;

  // CASE 4.: 'buf[0]' becomes indirectly tainted as the address 'buf + w'
  // depends on the value of the tainted location 'w'.
  buf[0] = buf[w] - 1;
}
\end{listing-nonumber}
