...
\section*{33.0 (Arsenic)}

\begin{itemize}
  \item \textbf{Testing mode}
    document the new testing mode of \framac for runtime tests.
  \item \textbf{Ptests ENV directive}
    document the new \texttt{ptests} \texttt{ENV} directive.
\end{itemize}

\section*{32.0 (Germanium)}
\begin{itemize}
  \item \textbf{Logging Services}
    document font styling and colors for terminal outputs
  \item \textbf{Command-line Options}:
    document function \texttt{Parameter\_customize.is\_unsafe}
  \item \textbf{Ptests}: Deprecate \texttt{PTEST\_RESULT} and introduce
    \texttt{PTEST\_RESULT\_DIR} instead.
  \item \textbf{Ptests}: Introduce \texttt{PTEST\_RESULT\_FILE} representing the
    basename of the result file of the current test (only usable in
    \texttt{FILTER} macro).
\end{itemize}

\section*{31.0 (Gallium)}
\begin{itemize}
  \item \textbf{OCamlmig}: Document the usage of \texttt{OCamlmig} for
    migrations.
\end{itemize}
