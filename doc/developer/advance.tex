...
are really relevant. They are defined in modules
...
\item Type \texttt{Addresses.Bytes.t}\scodeidxdef{Addresses}{Bytes.t}
  is used to represent values of \C expressions like \texttt{2} (an
  absolute address) or \texttt{((int) \&a) + 13}. With this
  representation, there is no way to know the size of a value while it
...
\item Type \texttt{Locations.t}\scodeidxdef{Locations}{t} is used to represent the
  right part of a \C affectation (including bitfields). It is represented by a
  \texttt{Addresses.Bits.t} (see below) attached to a size. It is possible to
