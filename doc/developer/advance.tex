...
Plug-in developers usually just need to copy and paste the existing Makefile
template and rename the opam file name (\verb|frama-c-<plugin>.opam|).

This Makefile reuses code from other Makefiles, distributed in Frama-C's
{\em share} directory, such as \verb|Makefile.testing|,
\verb|Makefile.installation|, etc. Their names indicate the kinds of features
they provide. Many are shortcuts for specific dune targets, but they also
provide linting and copyright headers-related features.
...
\item The syntax for \texttt{EXECNOW}\nscodeidx{Test!Directive}{EXECNOW}
  ...
  described by \texttt{EXECNOW} directives are executed in
  order and
  before running any of the other directives. If the execution of
  one \texttt{EXECNOW} directive fails ({\it i.e.} has a
  non-zero return
