...
When the default behavior is missing in a function contract but needed by either
\framac or a plug-in, it can call \verb+Populate_spec.populate_funspec+%
...
This code will generate specifications for the function \verb+kf+ using the
selected mode (see the user manual~\cite{userman} for more details on how a mode
is selected). The parameter \verb+do_body+ (\verb+false+ by default) is used to
chose if we want to generate specification only on prototypes or also for
functions with a body. We also give a list of clauses for which we want to
generate some specifications. Here we only want to generate \verb+`Assigns+ and
\verb+`Exits+ clauses, but \verb+`Requires+, \verb+`Allocates+ and
\verb+`Terminates+ are also available. This function can take an optionnal
argument \verb+loc:Cil_types.location+ which is a location used when emitting
missing specification warnings. By default this location is set to the location
of \verb+kf+.

The generated specifications either come form nothing (generated using the
selected mode) or by combining existing clauses from other behaviors.

\subsection{Custom mode registration}

If none of the available modes behave as needed, it is also possible to create
a customized mode. For this, we need to define generation functions for each
clause:
...
command line options \verb+-generated-spec-mode mymode+. All parameters are
optional and, if a generation function is left unspecified, \verb+Frama_C+ mode
(see the user manual~\cite{userman}) will be used instead to generate the
corresponding clauses (emits a warning). It is also possible to specify a
property status\codeidx{Property\_status} to be emitted when a clause is
generated (emits a warning if omitted). Requires are the only clause for which
it is not possible to specify a status, because
\texttt{Populate\_spec}\codeidx{Populate\_spec} will never try to emit status of
requires.
