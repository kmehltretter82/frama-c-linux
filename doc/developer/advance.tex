...
\section{Plug-in Migration from the previous version of \framac }\label{adv:framac-migration}

\begin{target}developers who have an existing plug-in for Frama-C 30 or newer and
  want to migrate their plug-in to the next version.
\end{target}

\begin{prereq}
  Being familiar with the plug-in to migrate. Using \ocaml 5.2.1 or higher.
  Having installed \texttt{OCamlmig} (see below).
\end{prereq}
...
\subsubsection{\texttt{OCamlmig}\codeidxdef{OCamlmig}}
...
scripts were very incomplete and when \texttt{OCamlmig} (see
\url{https://github.com/v-gb/ocamlmig}) was first released early 2025, we
decided to have look at it to help automating migrations. \framac
developers can now indicate in the code how it should be migrated. Plug-in
developers then can run \lstinline{ocamlmig migrate -w} inside their plug-in
directory after installing \framac and it will apply all the migration rules in
their code.

\subsubsection{Format}

At the moment, \texttt{Ocamlmig} uses \texttt{OCamlFormat} to print the code. If
you do not use \texttt{OCamlFormat} in your plug-in, you can minimize the diff
by setting an environment variable \texttt{OCAMLMIG\_PRESERVE\_FORMAT} before
running the migration command, either by exporting it via
\lstinline{export OCAMLMIG_PRESERVE_FORMAT=1} or in front of the command
\lstinline{OCAMLMIG_PRESERVE_FORMAT=1 ocamlmig migrate -w}. With this setting,
\texttt{OCamlmig} will try to format only chunks of codes that are impacted by
the tool (without any guarantee that the diff will be minimal, but, in practice,
the results usually look right).

\subsubsection{Incremental Migration}

It must be noted that the \texttt{migrate} attributes for \texttt{ocamlmig} are
...
of Frama-C, you will need to make the migration step by step for each
...
The full list of supported tags is available in
