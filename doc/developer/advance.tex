...
%%item
  ...
  and thus aborts the computation.
...
Such clauses can be found in two places: in a contract, where they are
stored in the \texttt{b\_extended}\sscodeidx{Cil\_types}{behavior}{b\_extended} field of
\texttt{Cil\_types.behavior}\scodeidx{Cil\_types.behavior}, or
in a loop annotation, a special form of
\texttt{code\_annotation}\scodeidx{Cil\_types}{code\_annotation} introduced by the
\texttt{AExtended} constructor. Such annotations
must be introduced by a keyword \texttt{kw}
(or \texttt{loop kw} for loop annotations) that will be used to identify them
in the AST. An \texttt{acsl\_extension}\scodeidx{Cil\_types}{acsl\_extension} is a pair
\texttt{(kw, ext)} where \texttt{kw} identifies the extension and ext is an
...
registered by the
\texttt{Logic\_typing.register\_behavior\_extension}%
\scodeidx{Logic\_typing}{register\_behavior\_extension}
function.
After a call to
\verb|Logic_typing.register_behavior_extension kw f|,
...
as belonging to the extension \verb|kw|. Extensions clauses must occur after
\verb|assumes| and \verb|requires| clauses if any, must can be freely
...
other loop annotations clauses. Extensions can be made specific to a
...
extension is supposed to be considered only when behavior \verb|bhv| is
...
with the current typing environment (which allows to discriminate
between contract and loop extension). \verb|f| must return the corresponding
\verb|acsl_extension_kind| (possibly adding a entry for key \verb|id|
