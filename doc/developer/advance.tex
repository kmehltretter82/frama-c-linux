...
\begin{target}developers who have an existing plug-in for Frama-C 31 or more and
  want to migrate their plug-in to the next version.
\end{target}

\begin{prereq}
  Being familiar with the plug-in to migrate. Using \ocaml 5.2.1 or higher.
  Installing \texttt{OCamlmig}\codeidxdef{OCamlmig}.
\end{prereq}
...
scripts were not reliable and we decided to use a \texttt{OCamlmig} (see
\url{https://github.com/v-gb/ocamlmig}), to automate migrations. \framac
developers can now indicates in the code how it should be migrated. Plug-in
developers then can run \lstinline{ocamlmig migrate -w} inside their plug-in
directory after installing \framac and it will apply all the mirgation rules in
their code.
...
the tool without any guarantee.
