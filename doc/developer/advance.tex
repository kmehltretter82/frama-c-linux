...
& Export an environnement variable for \texttt{OPT}, \texttt{STDOPT} or
...
\item The \texttt{ENV}\nscodeidx{Test!Directive}{ENV} directive
  ...
  \end{code}
  where \texttt{variable-name} is any valid environnement variable name, and
...
\item The \texttt{FILTER}\nscodeidx{Test!Directive}{FILTER} directive specifies
  ...
  \begin{code}
FILTER: diff --new-file %{dep:@PTEST_SUITE_DIR@/result_config/@PTEST_RESULT_FILE@} -
  \end{code}
...
\hline
\verb|PTEST_RESULT_FILE| & basename of the current result file (macro only usable in FILTER directives)\\
\hline
