...
have \ocpindent installed to launch these targets.
...
In this section, we present how \framac can be tested, notably using \ptests, a
tool provided by \framac in order to perform non-regression and unit tests.
...
\begin{example}
  Test \texttt{plugins/sparecode/tests/sparecode/calls.c} declares the
...
& Command reading the standard input used to filter results.
  In such a command, the predefined macros \texttt{@PTEST\_ORACLE@} and
  \texttt{@PTEST\_RESULT\_FILE@} are set respectively to the basename of the
  oracle and the result.
& \textit{None}
\\
\hline
...
\\
\hline
\texttt{ENV}\nscodeidxdef{Test!Directive}{ENV}
& Export an environment variable for \texttt{OPT}, \texttt{STDOPT} or
\texttt{EXECNOW} directives.
& \textit{None}
\\
\hline
...
\item The \texttt{ENV}\nscodeidx{Test!Directive}{ENV} directive
  has the following syntax:
  \begin{code}
    ENV: variable-name value
  \end{code}
  where \texttt{variable-name} is any valid environment variable name, and
  \texttt{value} extends until the end of the line. Each encountered
  \texttt{ENV} directive will export the variable which will be visible in the
  context of \texttt{OPT}, \texttt{STDOPT} or \texttt{EXECNOW} directives. A
  variable can be un-set via \texttt{ENV: variable-name} or \texttt{ENV:
  variable-name ""}. Declaration order matters, and an \texttt{ENV} directive
  can be used to override a previous one on the same variable. Macros can be
  used as values for this directive and they will be expanded before exporting
...
\item The \texttt{FILTER}\nscodeidx{Test!Directive}{FILTER} directive specifies
  ...
  In such a directive, the predefined macros \texttt{@PTEST\_ORACLE@} and
  \texttt{@PTEST\_RESULT\_FILE@} are set respectively to the basename of the
  oracle and the result.
  That allows running a \texttt{diff} command with the oracle or result of
  another test configuration \texttt{config}:
  \begin{code}
FILTER: diff --new-file %{dep:@PTEST_SUITE_DIR@/result_config/@PTEST_RESULT@} -
  \end{code}
  It is recommended to run \texttt{diff} against \texttt{@PTEST\_RESULT\_FILE@}
  so that the dependencies between test configuration are correctly set up by
  the build system.
...
Name & Expansion \\
\hline
\verb|frama-c-exe| & path to \framac executable \\
\hline
...
\hline
\verb|PTEST_RESULT_DIR| & shorthand alias to \verb|@PTEST_DIR@/result@PTEST_CONFIG@|
...
\hline
\verb|PTEST_RESULT| & basename of the current result file (macro only usable in FILTER directives)\\
\hline
...
\subsection{Testing mode for runtime test execution}\label{sec:testing-mode}

\nscodeidxdef{Test}{FRAMAC\_TESTING}
\framac uses the environment variable \texttt{FRAMAC\_TESTING} to determine
whether it is running inside a test or not. When set to \texttt{yes},
\framac will execute optional inlined test code at runtime, which can
be useful to perform additional tests and verification (even costly).

There are several ways to set this variable, depending on the test framework
used: via \ptests \texttt{ENV} directive, via dune \texttt{setenv} action or
manually exporting this variable before running \framac. It must be set to
\texttt{yes}.

\scodeidxdef{Cmdline}{inside\_tests}
This variable controls the value of \texttt{Cmdline.inside\_tests}, which
is available in \framac's API and can be used to guard tests, for example:

\begin{lstlisting}[language=OCaml]
(* Lets say add has some preconditions, which in practice are
    supposed to be always true, so we don't want to impact performances
    and verify everytime. *)
let add x y =
  if Cmdline.inside_tests then
    (* Only check preconditions during tests *)
    assert (x > 0 && y < 100);
  x + y
\end{lstlisting}

During tests, this assertion will be checked for each call, and in normal
execution we ignore it.
...
When working in the Frama-C \texttt{plugins} directory, enabling or
...
\section{Plug-in Migration from the previous version of \framac with \ocamlmig }\label{adv:framac-migration}

\begin{target}developers who have an existing plug-in for Frama-C 30 or newer
  and want to migrate their plug-in to the next version.
\end{target}

\begin{prereq}
  Being familiar with the plug-in to migrate. Using \ocaml 5.2.1 or higher.
  Having installed \ocamlmig (see below).
\end{prereq}

\begin{important}
  Please note that this section is a best effort procedure for making the
  migration as smooth as possible. If the plug-in is particularly complex,
  please contact us if you need some help for migrating it. Note also that
  \texttt{ocamlmig} is very young, and even though it has been successfully
  tested on the whole Frama-C codebase, some issues might still be present.
\end{important}

Previous version of \framac used migration shell scripts and \texttt{sed} to
migrate things that changed in the API (like moving or renaming functions). This
scripts were very incomplete and when \ocamlmig (see
\url{https://github.com/v-gb/ocamlmig}) was first released early 2025, we
decided to have look at it to help automating migrations. \framac developers can
now indicate in the code how it should be migrated.

\subsubsection{Usage}

Plug-in developers can run \lstinline{ocamlmig migrate -w} inside their plug-in
directory after installing \framac and it will apply all the migration rules in
their code.

\subsubsection{Format}

\ocamlmig can break \ocaml formatting when printing the replaced code, you may
need to use \ocpindent to fix this, \lstinline{make check-lint} will tell which
files are not formatted correctly according to \ocpindent.

\subsubsection{Incremental Migration}

It must be noted that the \lstinline{migrate} attributes for \ocamlmig are
meant to perform a migration from \framac version N to \framac version N+1 (or
from the last stable release to the development branch). In case you're trying
to migrate a plug-in that has not been maintained for several releases
of \framac, you will need to make the migration step by step for each
...
warning, but this can be changed via the optional parameter \lstinline|default|
of \lstinline|register_warn_category|, or by using the
...
The code samples below show how to use and set the current location.
  ...
  let print_current_loc () =
    Format.printf "%a@." Fileloc.pretty (Current_loc.get ())
...
\subsection{Font styling and Colors}

When output is directed to a terminal, \framac leverages the terminal's support
for \href{https://en.wikipedia.org/wiki/ANSI_escape_code}{ANSI escape codes} to
apply font styles and colors. This allows developers to enhance output
appearance using
\href{https://ocaml.org/manual/5.4/api/Format.html#tags}{semantic tags} with the
standard formatter.

For example, to print a word in bold, you can write:

\begin{ocamlcode}
Format.printf "this will output the word @{<bold>bold@} with a bold font"
\end{ocamlcode}

Semantic tags are opened and closed using \texttt{@\{<tag>} and \texttt{@\}},
where \texttt{<tag>} is one of the supported styling tags. The formatter
automatically translates these tags into the appropriate ANSI escape codes when
the output device supports them; otherwise, they are ignored.

Semantic tags work with all logging functions. When logs are written both to the
terminal and to a file (with a parameter \texttt{-<plugin>-log}), only the
terminal output will render the styles--the file output will contain plain ASCII
text.

\begin{ocamlcode}
Self.result "Hello, this is @{<bold>Foo@} Logs !"
\end{ocamlcode}

This will display \texttt{Foo} in bold on the terminal, but without formatting
when redirected to a file.

You can also combine multiple tags by separating them with commas. For example,
to print bold red text:

\begin{ocamlcode}
Self.result "Hello, this is @{<bold,red>Foo@} Logs !"
\end{ocamlcode}

The list of supported tags is available in
\texttt{src/libraries/utils/ansi\_escape.mli}.
Plugins may also extend and manage the set of supported semantic tags
using this module.

The command line option \texttt{-tty-debug} (hidden for non-developer users)
can be used to display non-supported semantic tags like HTML tags in the
standard output.
...
%%item
  ...
  any. Plug-in channels are registered under their short-name, and the kernel
  channel is registered under \lstinline{Kernel_log.kernel_channel_name}.
...
\begin{description}
%% item
  \logroutine{set\_formatter}{ ?isatty fmt}%
  Redefines the formatter the logs are written to. The formatter can be
  built with \lstinline{Format} functions like \lstinline{make_formatter}
  \lstinline{formatter_of_out_channel} or \lstinline{formatter_of_buffer}.
  Every \lstinline{Log} function will output to this formatter instead of
  \lstinline{Format.std_formatter} after this function have been called. Copied
  outputs to file (with \lstinline{-plugin-log} parameters) will still be
  copied. This function takes an optional \lstinline{isatty} parameter; set it
  to true if the redirection target can interpret ANSI escape codes.
  The original formatter can be reset with \lstinline{reset_stdout}.

%% item
  \logroutine{reset\_stdout}{ ?isatty ()}%
  Reset the logs formatter to \lstinline{Format.std_formatter}. This function
  should be called to restore the original state of \lstinline{Log} when
  \lstinline{set_formatter} has been previously called. The optional parameter
  should always be set to \lstinline{Cmdline.tty}.
\end{description}
...
\item It is automatically updated whenever the current project
  ...
  \begin{figure}[htbp]
    \centering
    \includegraphics[viewport=0 0 440 246,width=0.99\textwidth]{mechanism.pdf}
...
\noindent
\listingname{plugins/from/src/functionwise.ml}
...
let self = Tbl.self
\end{ocamlcode}
\listingname{plugins/from/src/from.ml}
\begin{ocamlcode}
...
\noindent\listingname{plugins/pdg/src/pdg\_tbl.ml}
\scodeidx{Kernel\_function}{Make\_Table}
\begin{ocamlcode}
module Tbl =
  Kernel_function.Make_Table
...
let self = Tbl.self
\end{ocamlcode}
\listingname{plugins/pdg/src/register.ml}
...
copy. Invariant~\ref{eq:independence} is a local independence criterion which
...
\begin{example}
  ...
  \texttt{plugins/occurrence/src/options.ml}). It is implemented as follows.
...
\texttt{plugins/impact/src/options.ml}). It is implemented as follows.
...
\begin{important}
In particular, options that could be used to make \framac launch arbitrary
commands (such as \lstinline|-cpp-command|) or execute arbitrary code
(such as \lstinline|-load-module|) should be marked as unsafe using function
\texttt{Parameter\_customize.is\_unsafe}\scodeidxdef{Parameter\_customize}{is\_unsafe}
\end{important}
...
generate an \texttt{YAML} file with \texttt{frama-c-script make-machdep}.
This Python script requires a
C11-compliant cross-compiler (ideally, with a standard library, i.e, {\em not}
a freestanding target) for the architecture you want to describe.
...
Each field of the machdep is succinctly described in the
...
\begin{description}
  ...
  \item[\texttt{alignof\_aligned}]: the default alignment of a type having the
    \verb+aligned+ attribute (or negative if unsupported). This corresponds to
    the default alignment when using \verb+#pragma packed()+ without a numeric
    argument.
\end{description}
...
\begin{description}
  ...
  \item[\texttt{posix\_c\_source}]: value of the \texttt{\_POSIX\_C\_SOURCE} macro.
...
There are four different representations of \C locations.
They are defined in modules
\texttt{Locations}\codeidxdef{Locations},
\texttt{Addresses}\codeidxdef{Addresses} and
\texttt{Memory\_zone}\codeidxdef{Memory\_zone}. They are introduced below.
See the documentation in the following files for details about the
provided operations on these types:
\begin{itemize}
  \item For \texttt{Locations}:
    \texttt{src/kernel\_services/abstract\_interp/locations.mli};
  \item For \texttt{Addresses}:
    \texttt{src/kernel\_services/abstract\_interp/addresses.mli};
  \item For \texttt{Memory\_zone}:
    \texttt{src/kernel\_services/abstract\_interp/memory\_zone.mli}.
\end{itemize}

The four representations are:
\begin{itemize}
\item Type \texttt{Addresses.Bytes.t}\scodeidxdef{Addresses}{Bytes.t}
  is used to represent sets of values of \C pointers like \texttt{(int*)2}
  (an absolute address), \texttt{\&a + 13} or any pointer \texttt{p}. With
  this representation, there is no way to know the size of a value while it
  is still possible to join two values. Roughly speaking it is
  represented by a mapping between \C variables and offsets in bytes.
\item Type \texttt{Addresses.Bits.t}\scodeidxdef{Addresses}{Bits.t} is
  similar to \texttt{Addresses.Bytes.t} with offsets in bits instead of
  bytes. It should only be used inside a location (see below).
\item Type \texttt{Locations.t}\scodeidxdef{Locations}{t} is used to represent
  the set of bits accessed through a \C lvalue (including bitfields). It is
  represented by a \texttt{Addresses.Bits.t} attached to a size. It is possible
  to join two locations \emph{if and only if they have the same sizes.}
\item Type \texttt{Memory\_zone.t}\scodeidx{Memory\_zone}{t} is a set of bits (without
