...
Concerning code formatting, the \framac team currently uses the \ocpindent opam
package for code indentation. Consider installing it if you want to ensure your
...
\item \texttt{name} is an arbitrary, non-empty string containing the full name
  of the plug-in. It must start with an uppercase letter.
\item \texttt{shortname} is a small, non-empty string containing the {\em short
  name} of the plug-in, to be used as a prefix for all plug-in
  options\footnote{\framac does not enforce that all plug-in options are
  prefixed with its shortname, but it is customary to do so.}. It must start
  with a lowercase letter and only contain lowercase letters and numbers,
  possibly separated by \texttt{-} or \texttt{\_}.
...
\begin{important}
  ...
  prefer using Dune's test system directly.
\end{important}
...
\$ dune build @ptests
Files ../oracle/hello_test.0.res.oracle and hello_test.0.res.log differ
% dune build @"tests/hello/result/hello_test.0.exec.wtests": diff failure on diff:
(cd _build/default/tests/hello/result && diff --new-file "hello_test.0.res.log"
"../oracle/hello_test.0.res.oracle")
File "tests/hello/oracle/hello_test.0.res.oracle", line 1, characters 0-0:
diff --git 1/_build/default/tests/hello/oracle/hello_test.0.res.oracle
            2/_build/default/tests/hello/result/hello_test.0.res.log
index e69de29..5f6ab23 100644
--- 1/_build/default/tests/hello/oracle/hello_test.0.res.oracle
+++ 2/_build/default/tests/hello/result/hello_test.0.res.log
@@ -0,0 +1,2 @@
...
\item Dune compared two files, none of which existed before running the test:
  \verb|result/hello_test.0.res.log| and \verb|oracle/hello_test.0.res.oracle|;
...
The first file, \verb|result/hello_test.0.res.log|, is the {\em actual} output of
the execution of the test command.
The second file, \verb|oracle/hello_test.0.res.oracle|, is the {\em expected}
output of the test.
...
<...>
\$ dune build @ptests
Files ../oracle/hello_test.0.res.oracle and hello_test.0.res.log are different
% dune build @"tests/hello/result/hello_test.0.exec.wtests": diff failure on diff:
 (cd _build/default/tests/hello/result && diff --new-file "hello_test.0.res.log" "../oracle/hello_test.0.res.oracle")
File "tests/hello/oracle/hello_test.0.res.oracle", line 1, characters 0-0:
diff --git 1/_build/default/tests/hello/oracle/hello_test.0.res.oracle 2/_build/default/tests/hello/result/hello_test.0.res.log
...
plug-in with \ptests. Small plug-ins such as \texttt{Report} are good examples
(see directories \texttt{src/plugins/report/tests}).
...
Extending \framac's graphical user interface is a task too large for
this tutorial; the GUI being a desktop Electron application, written in
