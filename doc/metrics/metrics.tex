...
Only cyclomatic numbers are available for the normalized AST.
...
\begin{figure}
  ...
  {../../src/plugins/metrics/tests/metrics/\pgname}
...
cyclomatic complexity are computed.
...
This information is computed both for the complete source code, and
on a per-function basis -- except for the number of functions
declared.

Cyclomatic complexity is then derived from this information, both for the
...
Halstead complexity measures first need the following information from the
source code:
\begin{itemize}
...
From the above information, Halstead defines the following measures:
\begin{center}
  \begin{tabular}{lcl}
