...
coverage estimate. This item is detailed in Section \ref{sec:value-analysis-cover}.

\item[-metrics-value-cover] activates the value coverage estimation.
  This item is detailed in Section \ref{sec:value-analysis-cover}.
...
\subsection{Value analysis coverage}
\label{sec:value-analysis-cover}

The {\sf -metrics-value-cover} option can be used to compare the code
effectively analyzed by the Value Analysis with what {\sf Metrics}
...
\begin{shell}[escapechar={\#}]
%  frama-c -metrics -metrics-value-cover #\pgname#
\end{shell}

Syntactic reachability is an over-approximation of what is actually
reachable by the value analysis. Thus, the coverage estimation will
...
are not analyzed by the value analysis, the plug-in indicates the
...
%
Finally, the plug-in displays the percentage of statements analyzed by Value
for each function.

\begin{figure}[!ht]
  \centering
    \lstinputlisting[language=Logs, basicstyle=\lp@basic, firstline=42]{./cover.log}
  \caption{Value coverage estimate for \pgname}
...
\section{Value coverage}

This part of the Metrics plug-in is thought of as a help for new code
exploration with Frama-C's value analysis plug-in. The first steps into a new
code can
be quite complicated and, more often than not, the value analysis stops from too
...
The graphical user interface helps visualizing where the value analysis has
stopped. The penetration estimate of Metrics aims to complement that by giving a
rough approximate of the percentage of code that the value analysis has
seen. This is especially interesting when comparing two runs of the value
analysis on the same code, after performing some tweaks and improvements to help
the value analysis: one can thusly quantify the added penetration due
to the tweaks.
