...
version, you can find installation instructions at \url{https://github.com/jgm/pandoc/blob/master/INSTALL.md}
\item \texttt{tidy}
...
\item In case a new release of ACSL is done together with Frama-C's release;
  \begin{itemize}
  \item add \texttt{distrib/download/acsl\_X\_Y.pdf} (using
    \texttt{git annex add}; \todo{Fait par le script \texttt{build-src-distrib.sh}? Check with FB}
  \item update \texttt{www/download/acsl.pdf};
...
\item Update the \texttt{Makefile} in \texttt{src}.
  Add the new files to export (if any) in the \texttt{PAGES} variable of the
  \texttt{Makefile}.
...
\item Update also some macros of \texttt{download.def} file:
  \begin{enumerate}
  \item Add an entry into \texttt{selector-previous-versions} macro.
...
\item Build the web site:
  \begin{shell}[gobble=4]
    ...
    \$ make distrib
  \end{shell}
  You get a snap-shot of the repository in the \texttt{www} directory.
  Test it carefully.\\

  Automatic testing for compliance to \textsf{W3C} and links consistency
  was launched (install the required tools before if necessary).
  \textbf{You must check that all the verifications are ok:}
  \begin{shell}
    \$ make check_link
  \end{shell}
...
You'll need a GitHub account to create a pull request.
...
\item Create a pull request to opam-repository
\end{itemize}

\section{Updating Frama-C-snapshot}

You'll need a GitHub account with commit rights to Frama-C/Frama-C-snapshot.
The release script \texttt{bin/build-src-distrib.sh} has prepared both the
repository and the associated wiki (see section~\ref{sec:build-source-distr}).

\begin{itemize}
\item ensure that the sources in \texttt{Frama-C-snapshot} compile (\texttt{./configure --prefix=/tmp/frama-c \&\& make \&\& make install} must work).
...
\expertise{Usually Julien performs this step.}
...
Just update the \texttt{VERSION} file in \texttt{trunk}, by adding
