...
Create the milestone for the next releases on \textsf{GitLab}, in the Frama-C
group. They will be used for development that will not be integrated into the
upcoming release. This must be done during CI/CD meeting.
...
\section{Stable branch CI}
\label{sec:stable-branch-ci}

When the stable branch is created, nightly pipelines must be enabled for the
stable branch. Ask \expertise{Allan} to enable the pipelines. For this, on
GitLab, go to Build, Pipeline Schedules and edit the two ``Stable'' pipelines:
\begin{itemize}
  \item change ``target branch'' to the new stable branch,
  \item tick the ``Activated'' parameter.
\end{itemize}

These pipelines must be disabled after the release.
