...
\item precondition checking ({\it requires} and {\it assumes} clauses) at
  function's call sites, as well as postconditions ({\it ensures} clauses) and
  {\it assigns}, for functions having an ACSL specification.
\end{itemize}
...
\item generating call sites statement contracts, based on the called function's
  contract. This is useful for modular verification.
...
\chapter{Precondition annotation generation}

%%\section{Preconditions at call sites}
Using ACSL, the Frama-C frameworks allows the user to write contracts for C
functions. These contracts contain in particular:

\begin{itemize}
\item \lstinline|requires| clauses \lstinline|$R_1$, $R_2$, $\ldots$|
  (preconditions),
\item \lstinline|ensures| clauses \lstinline|$E_1$, $E_2$, $\ldots$|
  (postconditions),
\item \lstinline|assigns| clauses \lstinline|$A_1$, $A_2$, $\ldots$| (set of
  locations assigned by the function).
\end{itemize}

 The intended behavior is that any caller must make sure that the function is
 called in a state where the required property 
\lstinline|$R_1$ && $R_2$ && $\ldots$| holds, to ensure that
 \lstinline|$E_1$ && $E_2$ && $\ldots$| holds in 
 the state returned by the function call.  Contracts may contain several named
 behaviors as well, see the ACSL manual.

For a function having a contract, the \rte{} plug-in generates a statement
annotation corresponding to the contract at each call site, if the option
\texttt{-rte-precond} is specified.  It does so by substituting formal
parameters of the contract with actual parameters at the call site.  The
generated statement annotation ensures that the function is called in the proper
state and specifies its return state.

\begin{example} ~

Consider function \lstinline|f| with the following contract:

\listinginput{1}{fcontract.c}

Here is the output of \rte{} with the option \texttt{-rte-precond}: 
\listinginput{1}{stmtannot.c}

The generated behaviors names are \lstinline|pre_f, pre_f_pos, pre_f_neg|).
Notice that \lstinline|assigns| clauses are also taken into account, and that
the formal \lstinline|\result| is substituted with the assigned left-value.

\end{example}

The main restriction is that the \rte{} plug-in only performs syntactic
detection of function calls: functions called through pointers are warned about
but no statement annotation is generated. This is unlikely to change until ACSL
introduces contracts on function pointers.

%% Another restriction concerns statements in which the returned value of a
%% function is assigned to a left value with an intermediate cast. In that case, no
%% statement annotation is generated and a warning is issued as can be checked on
...
%% > frama-c -rte -rte-precond castoncall.c

%% [kernel] preprocessing with "gcc -C -E -I.  /tmp/castoncall.c''
%% [rte] annotating function main
%% /tmp/castoncall.c:9:[rte] warning: (c = (char)f();) function call with 
%% intermediate cast: not treated
%% \end{listing-nonumber}

%% This comes from the fact that by default, the statement \lstinline|c = f();|  is
%% translated to 

%% \lstinline|c = (char) f();|

%% To overcome this restriction, use the \lstinline|-no-collapse-call-cast|
%% option. This will transform the statement into an equivalent sequence on
%% which the annotation generation won't fail:
...
%%Adding annotation based on preconditions at functions call sites.
%%Preconditions usually refer to the function's formal parameters (and may also refer to global variables, 
%%which are not a problem w.r.t. annotation generation), hence the assertion to be checked at a function's call site 
%%is obtained by substituting the formal parameters with the actual parameters used for the call. 
...
\cnn{}. In a similar way, \lstinline|-rte-precond| must be explictly used to
generate precondition annotations.
...
instance, used in conjunction with \lstinline|-rte-precond| alone, only
precondition annotations would be generated. Adding \lstinline|-rte-mem|,
annotations for the validity of memory access would also be generated.
...
\lstinline|frama-c -rte -rte-select f,g -rte-no-all -rte-precond|: only generate
precondition annotations, and only for call-sites found in functions
...
\texttt{frama-c -rte -warn-unsigned-overflow -rte-precond
  -rte-no-trivial-annotations}: generate all possible annotations (unsigned
overflows and precondition annotations included), but do not try to evaluate
their status through constant folding.
...
\hline
\lstinline |-rte-precond| & boolean (false) & Generate contract-based statement
behaviors based at call sites \\
\hline
