...
variable sized arrays: see the example in \iso{6.5.4.4}{8}.
...
indeterminate value (\iso{6.7.11}{11}). Such a value is either an unspecified
value or a trap representation. Only reading a trap representation is an
undefined behavior (\iso{6.2.6.1}{5}). It corresponds to the second case above.
...
\item Demotion of a real floating type to a smaller floating type
  producing a value outside of the representable range (\iso{6.3.1.5}{2})
\item Conversion between two pointer types produces a result that is incorrectly
  aligned (\iso{6.3.2.3}{7})
\end{itemize}
