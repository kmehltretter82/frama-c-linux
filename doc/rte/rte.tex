...
\item accesses to arrays that are embedded in a struct occur within valid
  bounds, this is stricter than verifying that the accesses occur within the
  struct.
\end{itemize}
...
\item[{\tt -rte-use-eva-results} (boolean (defaults to true))] \ \smallskip \\
     Do not generate annotations for functions analyzed by Eva. The option is
     not available when the Eva plug-in is not available at compile time.
\end{description}
...
\item[{\tt -safe-arrays} (boolean, defaults to true, opposite option is {\tt -unsafe-arrays})] \ \smallskip \\
     Generate annotations for index-out-of bounds of arrays inside structs.
\end{description}
