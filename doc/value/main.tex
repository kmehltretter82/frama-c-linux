...
annotation point, while a dynamic split enables a reasoning according to the
current value of the expression as long as the partitioning is active.

On both following examples, the values of the array \lstinline|t| are sorted in
...
On this second example, the value of \lstinline|r| is computed at each
loop iteration and depends on the current value of \lstinline|i|, which
also changes at each loop iteration.
So we need a dynamic split to ensures that the analysis separates the new
...
By default, the value partitioning made by \lstinline|split|
and \lstinline|dynamic_split| annotations is only maintained in the current
function: on the return to their calling functions, such value partitioning
...
of these. Arrays are summarized with only one abstract value, meaning the domain
can't express distinct properties of individual cells. However, for arrays
of structures, the field values are approximated field by field allowing the
inference of precise information about each field of the structures in the
array. The model also keeps track of the types used to write in the memory
which allows to display the contents of the memory (with
\lstinline|Frama_C_domain_show_each|) with it's structure, even if the
printed location's type doesn't match the type used to write into this location.

The domain may not be more precise than the cvalue domain, in particular when
array properties need to be enmurated cell by cell. However it may often be
more precise when large arrays of structures are written into, especially when
the indexes used for the write accesses cannot be reduced to singleton with
usual techniques like loop unrolling.
...
\lstinline|taints <lvalues>;| in function contract.

These directives are hypotheses of the taint analysis, and thus have no logical
status. The specified set of lvalues become tainted just after the code
...
The taint is then propagated during the Eva analysis, computing an an
