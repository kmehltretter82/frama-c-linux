...
When launched with \lstinline|frama-c -val rte.c|,
...
the command \lstinline|frama-c-gui -val *.c| launches
...
\begin{logs}
lib.c:17:[value] entering loop for the first time
\end{logs}
...
The command \lstinline|frama-c *.c -val -print| launches
...
\subsection{Increasing the precision with option {\tt -slevel}}
...
have to be the option \lstinline|-slevel|. This option has
two visible effects:
...
applying the \lstinline|-slevel| option must be tried first.
Indeed, \lstinline|-slevel| is one of the options of EVA
...
The \lstinline|-slevel| option is useful for unrolling loops.  We do
...
\lstinline|-slevel 100| should have a good chance of unrolling at
...
the parameter \lstinline|-slevel 100|, the precision of the analysis still isn't
...
main_1.c:14:[value] completely invalid value in evaluation of
                      argument (int)hash[i]
...
with \lstinline|-slevel 100| produces no alarms (apart from the postconditions
...
\begin{shell}
frama-c -slevel 100 -val *.c >log 2>&1
\end{shell}
...
dependency on global variable \verb|ONE| reveals an implementation
...
would be to try to change the value of \verb|ONE| between
computations.  Checking the source code for mentions of variable 
\lstinline|ONE|, we can see it is used to
...
Assuming that variable \lstinline|ONE| keeps its value, a same
...
at \url{http://blog.frama-c.com/index.php?tag/skein}; read 
available posts in chronological order.
...
The ISO C99 standard also forbids left-shifting a negative
integer. This is also detected by EVA.
...
%
Regardless of the value of option \lstinline|-warn-signed-overflow|,
a warning is emitted on signed arithmetic
operations applied to the cast to signed int of an address, since the
compiler may place variables in memory at will.
...
\lstinline|-warn-unsigned-downcast| are not honored by EVA.
...
Option \lstinline|-val-warn-copy-indeterminate| can be used to obtain a
more aggressive behavior on scalar variables. Specifying \lstinline|-val-warn-copy-indeterminate f|
...
option \lstinline|-undefined-pointer-comparison-propagate-all|. 
...
deactivated using option \lstinline|-val-warn-undefined-pointer-comparison|.
...
\begin{logs}
valid_function.c:9:[kernel] warning: pointer to function with incompatible type.
        assert \valid_function(p);
valid_function.c:9:[kernel] warning: Neither code nor specification for
        function f1, generating default assigns from the prototype
[value] using specification for function f1
valid_function.c:12:[kernel] warning: pointer to function with incompatible type.
        assert \valid_function(p);
valid_function.c:12:[value] assertion 'Value,function_pointer' got final status
        invalid.
\end{logs}
...
\lstinline|-undefined-pointer-comparison-propagate-all|.}
...
Calls to variadic functions are handled by the {\em Variadic} plug-in, which
...
in this manual, section \ref{trait_fonctions}.
...
and it is always possible to compute an offset to go from on address
...
it generates a proof obligation that means that ``the comparison
...
Each call to \lstinline|malloc| and \lstinline|realloc| potentially creates a
...
Dynamically allocated bases behave mostly like statically allocated one,
...
of \lstinline|x| is in  $\bropen 100..127\brclose$. This does
...
With the option \lstinline|-slevel| described in section \ref{slevel}, 
...
\lstinline|-value-help| lists options specific to the
EVA plug-in. The options \lstinline|-kernel-help| and
\lstinline|-value-help| also list advanced options that are
...
The \lstinline|-val| prefix found in most options is historical, and stems
from the former name of EVA (namely, Value).
...
The option \lstinline|-cpp-command <cmd>| sets the preprocessing command to use.
...
Option \lstinline|-val| activates EVA. Sets of
...
without it being necessary to provide the \lstinline|-val|
...
textual output. In particular, \texttt{-value-warn-key=-alarm} will completely
disable their output, and \texttt{-value-warn-feedback=alarm} will convert
them into normal messages instead of warnings (equivalent to deprecated option
\texttt{-no-val-warn-on-alarms}\footnote{Note that the use of
  \texttt{-no-val-warn-on-alarms -val-msg-key=-alarm} for completely
...
\lstinputlisting[linerange={9-15}]
...
write an analysis entry point that positions the
...
Option \lstinline|-context-depth| allows to specify this limit.
...
\lstinputlisting[linerange={7-13}]
...
\lstinline|-context-width| to one.  The value of the
option \lstinline|-context-depth| represents the length of the 
...
with options \lstinline|-context-width 1|
\lstinline|-context-depth 1|:

\lstinputlisting[linerange={7-17}]
...
The option \lstinline|-context-valid-pointers| causes
...
\lstinline|-context-width 1| \lstinline|-context-depth 1|
\lstinline|-context-valid-pointers|,
...
\lstinputlisting[linerange={7-12}]
...
\subsection{Treatment of loops}
\label{boucles-traitement}

\subsubsection{Default treatment of loops}
...
The solution in this case is to use one of the two unrolling options,
...
\subsubsection{Stipulating bounds}
...
\textbf{Note:} this syntax has been deprecated in favor of the previous one
...
\subsubsection{Loop unrolling}\label{deroulage}

There are two different options for forcing EVA
to unroll the effects of the body of the loop, as many times as
specified, in order to obtain a precise representation of the effects
of the loop itself. If the number of iterations is sufficient,
the analyzer is thus able to determine that each cell in the array
is initialized, as opposed to the approximation techniques
from the previous section.

\paragraph{Syntactic unrolling}

The option \lstinline|-ulevel n| indicates that Frama-C should unroll
the loops syntactically \lstinline$n$ times before starting any analysis.
If the provided number \lstinline|n| is larger than the number of iterations
of the loop, then the loop is completely unrolled and the analysis will
not observe any loop in that part of the code.

Providing a large value for \lstinline|n| makes the analyzed code bigger:
this may cause the analyzer to use more time and memory. This option
can also make the code notably bigger in presence of
nested loops (polynomially bigger in the depth of nested loops). A large value
should therefore not be used in this case.

It is possible to control the syntactic unrolling for each
loop in the analyzed code with the annotation
\lstinline|//@ loop pragma UNROLL n;|.  The user should place 
this annotation in the source code, just before
the loop. This annotation causes Frama-C's front-end to unroll 
the loop \lstinline|n| times.

\paragraph{Semantic unrolling}
\label{slevel}

The option \lstinline|-slevel n| indicates that the analyzer is allowed
...
iterations in the loop remain separated, as if the loop had been
unrolled.

The number \lstinline|n| to use depends on the
nature of the control flow graph of the function to analyze.
If the only control structure is a loop of \lstinline|m| iterations,
then \texttt{-slevel \textsf{m+1}} allows to unroll the loop completely.
...
option \lstinline|-slevel 55| in order to be completely unrolled:
\lstinputlisting[numbers=left]
...
\lstinputlisting[linerange={16-16}]
  {examples/parametrizing/slevel.1.log}

The number to pass the option \lstinline|-slevel|  is of the magnitude of
...
will only be precise for the first cells. The option \lstinline|-slevel 28|
gives for instance the following result for array \lstinline|t|:
\lstinputlisting[linerange={17-18}]
...
The option \lstinline|-slevel-function f:n| tells the analyzer to apply 
...
for instance \lstinline|-slevel-function crucial_initialization:2000|.
Oppositely, options \lstinline|-slevel 100 -slevel-function trifle:0| 
can be used together to avoid squandering resources over irrelevant parts of 
the analyzed application. The \lstinline|-slevel-function| option can be used 
several times to set the semantic unrolling level of several functions.


\subsubsection{Loop invariants}

A last technique, complementary to the unrolling ones above, 
consists in using the loop invariant construct of ACSL. Loop invariants
describe some
...
of \lstinline+-slevel+ or \lstinline+-ulevel+,
...
\subsection{Treatment of functions}
...
\lstinline|-no-results-function f| option.
The \lstinline|-no-results-function| option 
can be used several times, to omit the results of several functions.
The option \lstinline|-no-results| can be used to omit all
results.
The time savings can be important, as in the following example.

\lstinputlisting[numbers=left]
  {examples/parametrizing/nor.c}

\begin{shell}
time frama-c -val nor.c -slevel 2001
\end{shell}

\lstinputlisting[linerange={36-39},numbers=left,firstnumber=36]
  {examples/parametrizing/nor.1.log}
\lstinputlisting[linerange={2036-2041},numbers=left,firstnumber=2036]
  {examples/parametrizing/nor.1.log}
\lstinputlisting[linerange={4038-4042},numbers=left,firstnumber=4038]
  {examples/parametrizing/nor.1.log}

Launched with option \lstinline|-slevel 2001| so that the value returned
by \lstinline|main| can be computed precisely, the analysis takes 3 seconds.
 Contrast with:

\begin{shell}                                             
time frama-c -val nor.c -slevel 2001 -no-results-function irrelevant_function
\end{shell}

\lstinputlisting[linerange={36-40},numbers=left,firstnumber=36]
  {examples/parametrizing/nor.2.log}
\lstinputlisting[linerange={2037-2041},numbers=left,firstnumber=2037]
  {examples/parametrizing/nor.2.log}

When instructed, with 
option \lstinline|-no-results-function irrelevant_function|,  
that the values for the function
\lstinline|irrelevant_function| do not need to be kept, EVA
takes less than a second to produce its results. This shows
...
not influenced by the {\tt -no-results*} options. If you make use 
of only these functionalities, then you can speed up the analysis 
with option {\tt -no-results} without detrimental effects.}.
...
The meaning of option \lstinline|-no-results-function irrelevant_function| 
is different from that of option 
\lstinline|-slevel-function irrelevant_function:0|. In the case of the latter,
values
...
\subsubsection{Using the specification of a function instead of its body}
\label{val-use-spec}
...
If the solutions of the previous section are not sufficient, a radical
...
\verb+-val-use-spec f+. This instructs EVA to use the
...
Notice that option \verb+-val-use-spec f+ loses some guarantees. In particular,
...
\section{Analysis cutoff values}

\subsection{Cutoff between integer sets and integer intervals}

Option \verb|-val-ilevel <n>| is used to indicate the number of elements
...
available to improve the precision on specific code constructs. Theses
analyses are all experimental. We expect their exact perimeter to evolve
from one \FramaC version to the other.

The currently implemented domains offer no option other than the one
meant to activate them. They can (and probably should) be enabled at
the beginning of the analysis. Their only downside is that they
increase the analysis time.

\emph{Restrictions:}
\begin{itemize}
\item the results of theses analyses cannot currently be viewed in the GUI;
\item adding a new analysis may unfortunately change previous results in
  unpredictable ways, and new alarms may sometimes
  appear.\footnote{Adding now domains may reduce the convergence
    speed, and/or lead to more \lstinline+-slevel+ being consumed.
    Sometimes, those changes snowball into additional alarms.}
...
Activating option \texttt{-eva-equality-domain} instructs
...
Currently, the analysis is partially inter-procedural. The information
known in the caller is not propagated in the callee. On the other hand,
the information inferred in the callee is propagated back to the caller.
...
Activating option \texttt{-eva-symbolic-locations-domain} instructs
...
Activating option \texttt{-eva-gauges-domain} instructs EVA to
...
Activating option \texttt{-eva-bitwise-domain} instructs EVA to store
...
domain (with \lstinline+-slevel 2+).
\begin{lstlisting}
...
\begin{description}
\item[-eva-apron-box]: boxes/intervals
\item[-eva-apron-oct]:  octagons
\item[-eva-polka-equalities]:  linear equalities
\item[-eva-polka-loose]: loose polyhedra
\item[-eva-polka-strict]: strict polyhedra
\end{description}
...
When using {\em semantic unrolling} (section \ref{deroulage}), if an
...
value passed to option \lstinline|-slevel|) has not been reached yet
...
\begin{shell}
frama-c -val -slevel 2 sq.c
\end{shell}

The analysis finds the result
of this computation to be in \lstinline|[0..100]|. Without the
option \lstinline|-slevel 2|, or without the annotation on line 4, 
the result found
...
available, except when option \lstinline|-val-use-spec| is used
...
\lstinline|-no-val-skip-stdlib-specs|. Beware that when this option is
...
%
You can also use option \lstinline|-val-builtins-list| to obtain the list of
...
\lstinline|-val-builtin|, which takes pairs of functions:
the function to be replaced, and the name of the builtin that replaces it.
For instance, option
\lstinline|-val-builtin malloc:Frama_C_malloc_fresh,free:Frama_C_free| enables
builtins for the \lstinline|malloc| and \lstinline|free| functions of the
...
\lstinline|-no-val-builtins-auto|. You can then manually enable
\lstinline|-val-builtin| for each builtin that you do wish to activate.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{|c|c|c|}
    \hline
    C library function & Weak (always terminates) & Strong (more precise) \\
    \hline
    malloc & \lstinline|Frama_C_malloc_by_stack| & \lstinline|Frama_C_malloc_fresh| \\
    calloc & \lstinline|Frama_C_calloc_by_stack| & \lstinline|Frama_C_calloc_fresh| \\
    realloc & \lstinline|Frama_C_realloc| & \lstinline|Frama_C_realloc_multiple| \\
    free & \multicolumn{2}{c|}{\lstinline|Frama_C_free|} \\
    \hline
  \end{tabular}\label{tab:alloc}
  \caption{\FramaC builtins for dynamic allocation functions}
\end{table}

Note that function \lstinline|free| has a single builtin for both cases.

Generally speaking, the safest approach is to start with the builtins on the
left (\emph{weak}), to ensure that the analysis will terminate.
If the results are imprecise, and the allocation function is not called inside
a loop, then the strong variant may be tried. If you are mistaken, and the
...
[value] allocating variable __malloc_main_l42_2982
\end{listing-nonumber}

This indicates that new bases are being created.
The analysis will then have to be manually interrupted.
You will then need to either entirely unroll the loop ({\it e.g.} giving
the analysis enough \lstinline|slevel|), or use a weak variant.

By default, \emph{weak} builtins are used, but usage of
\lstinline|-val-builtin| takes precedence over preexisting associations.
Another possibility is to call e.g. \lstinline|Frama_C_malloc_fresh| manually,
for the allocations that are guaranteed to occur a finite number of times.


\paragraph{Multiple locations per callstack}

Option \lstinline|-val-mlevel| (default 0) sets the maximum number of calls to
...
In other words, setting \lstinline|-val-mlevel N| means that up to $N+1$
...
By default, the memory allocation builtins in EVA consider that
...
use option \lstinline|-no-val-alloc-returns-null|.
...
\begin{shell}
frama-c -val ex_nondet.c .../share/libc/__fc_builtin.c
\end{shell}
...
particular order during which the subsets composing these unions are propagated
by the analyzer.
...
option \verb+-value-msg-key category+, where \verb+category+ is the log category
of the domain, shown in Figure~\ref{fig:log-category}.

\begin{figure}
  \begin{centering}
    \begin{tabular}{lll}
      Domain & Log category & Ref\tabularnewline
      \midrule
      \midrule
      Symbolic equalities & \lstinline|d-eqs| & \ref{sec:symbolic-equalities}
      \tabularnewline
      \midrule
      Symbolic locations & \lstinline|d-symblocs| & \ref{sec:reuse}
      \tabularnewline
      \midrule
      ...
      \tabularnewline
      \midrule
      Apron binding & \lstinline|d-apron| & \ref{sec:apron}
      \tabularnewline
      \bottomrule
    \end{tabular}
    \par\end{centering}
    \caption{Log category of the additional domain \label{fig:log-category}}
...
through the option \verb+-value-msg-key category+, where \verb+category+ is the
...
\begin{table}[!ht]
  ...
  \end{tabular}
\end{table}
Note: \lstinline|rawmemchr| is a GNU extension to standard C.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{llll}
    ...
    \hline
    \lstinline|Frama_C_alloc_by_stack| &
    \lstinline|Frama_C_vla_alloc_by_stack| &
    \lstinline|Frama_C_malloc_fresh| \\
    \lstinline|Frama_C_calloc_by_stack| &
    \lstinline|Frama_C_calloc_fresh| &
    \lstinline|Frama_C_free| \\
    \lstinline|Frama_C_realloc| &
    \lstinline|Frama_C_realloc_multiple| &
...
\question{Which option should I use to improve the handling of loops
  in my program, {\tt -ulevel} or {\tt -slevel}?}

The options \lstinline|-ulevel| and \lstinline|-slevel| have different sets of
...
The \lstinline|-slevel| option does not allow to observe a specific
...
% The \lstinline|-slevel| option requires less memory than 
...
since it was an early draft. I am especially thankful to him for
...
getting the text to where it is today. Speaking of the text,
