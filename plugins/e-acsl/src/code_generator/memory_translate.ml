...
let rec has_set_as_index = function
  | TNoOffset ->
    false
  | TIndex(t, toffset) ->
    Ast_types.Acsl.is_plain_set t.term_type || has_set_as_index toffset
...
let range_to_ptr_and_size ~adata ~loc kf env ptr r p =
  ...
  in
  (* s *)
  let ty = match Ast_types.C.unroll_node (Misc.cty ptr.term_type) with
...
let term_to_ptr_and_size ~adata ~loc kf env t =
  let (e, adata), env =
    Env.with_params_and_result ~env (fun env ->
        ...
        (e, adata), env)
  in
  let ty = Misc.unghost_type @@ Misc.cty t.term_type in
             ...
             | TAddrOf(lh, off) ->
               begin match Logic_utils.last_term_offset off with
                 | TIndex({ term_node = Trange _ } as r, TNoOffset) ->
                   (* Case A *)
                   assert (Ast_types.Acsl.is_plain_set t.term_type);
