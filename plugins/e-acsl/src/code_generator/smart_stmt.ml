...
let do_call ~loc ?result vi args =
  ...
  let make_args ~variadic args param_ty =
    let rec make_rev_args res args param_ty =
      ...
      | arg :: args_tl, (_, ty, _) :: param_ty_tl ->
        let e =
          match ty.tnode, Ast_types.C.unroll_node (Cil.typeOf arg), arg.enode with
  ...
  in
  let args = match Ast_types.C.unroll_node vi.vtype with
...
let named_store_stmt name ?str_size vi =
  let ty = Ast_types.C.unroll vi.vtype in
  let loc = vi.vdecl in
...
let delete_stmt ?(is_addr=false) vi =
  ...
  match is_addr, Ast_types.C.unroll_node vi.vtype with
