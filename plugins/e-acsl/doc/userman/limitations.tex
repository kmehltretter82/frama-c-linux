...
big-endians, we recommend using the bittree-based memory model instead.
...
\begin{logs}
\$ e-acsl-gcc uninitialized.i -Omonitored_uninitialized
...
\begin{logs}
\$ e-acsl-gcc -n -M -omonitored_valid_no_main.i valid_no_main.c
...
While it is possible to add such instrumentation manually we recommend using
...
\begin{logs}
\$ e-acsl-gcc -n -M -omonitored_modified_main.i modified_main.c
\$ e-acsl-gcc -C -Ovalid_no_main monitored_modified_main.i monitored_valid_no_main.i
...
%% unspecified. The generated code is however easy to fix by hand.

\section{Variadic Functions}
\index{Function!Variadic}

Programs containing undefined variadic functions with contracts are not yet
supported. Using the \optionuse{-}{variadic-translation} option of \framac could
be a solution in some cases, but its generated code cannot always be compiled.
