...
\section*{E-ACSL 34.0 Selenium}

\begin{itemize}
\item \textbf{Optimization level} : Document new option
\texttt{-e-acsl-O} that configures the level of optimization for the resulting
code instrumented by \eacsl.
\item Renamed command-line flags
  \texttt{-e-acsl-valid} to \texttt{-e-acsl-O-valid},
  \texttt{-e-acsl-assert-print-data} to \texttt{-e-acsl-O-print-values},
  and \texttt{-e-acsl-gmp-only} to \texttt{-e-acsl-O-gmp-only}.
\end{itemize}


\section*{E-ACSL 33.0 Arsenic}

\begin{itemize}
\item Extended section \textbf{Integers} with more details, and
  a description of widening command-line flags.
\item Added section \textbf{Using \C functions in annotations} documenting the
  \texttt{-e-acsl-builtins} command-line flag.
\item Added documentation of the \texttt{-e-acsl-replace-libc-functions}
  command-line flag to the section \textbf{Incorrect Usage of Libc Functions}
