...
/*
 * Reduced current-Linux replay of __kvm_arm_vcpu_set_events().  Combining a
 * valid external-abort request with an invalid SError ESR commits exception
 * entry before the ioctl returns -EINVAL.
 */
...
static int set_events_before(struct kvm_vcpu *vcpu,
...
int main(void)
{
  ...
  int ret = set_events_before(&vcpu, &events);
