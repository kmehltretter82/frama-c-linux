...
let pre_analysis () =
  ...
  then begin
    Self.warning ~wkey:Log_key.warn_experimental
...
let print_shared_memory analysis =
  Self.result ~mkey:Log_key.shared_memory_zone
  ...
  Self.result ~mkey:Log_key.shared_memory_mutex
    "@[<v 2>Mutexes protecting access to shared memory:@ %a@]"
    Mt_mutexes_types.MutexesByZone.pretty mutexes;
  if Self.is_message_category_enabled Log_key.shared_memory_mutex_details then
    let protections = Mt_mutexes.check_protection analysis precise_accesses in
    Self.result ~mkey:Log_key.shared_memory_mutex_details
...
let print_data_races analysis =
  ...
  in
  Self.result ~mkey:Log_key.data_races
...
let post_analysis analysis =
  if not (Mt_thread.needs_recomputation analysis) then
    Self.feedback ~mkey:Log_key.thread_fixpoint
