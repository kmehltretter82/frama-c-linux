...
let generate_specs () =
  let aux kf =
    if need_assigns kf then begin
      Self.warning ~wkey:Key.warn_missing_assigns
...
let post_analysis (type t) (engine: t engine) mthread_analysis final_state =
  ...
  Self.warning ~wkey:Key.warn_garbled_mix_summary "%t" Origin.pretty_history;
...
let mthread_fixpoint engine analysis =
  ...
  Self.feedback ~dkey:Key.thread_fixpoint "First analysis of main thread";
