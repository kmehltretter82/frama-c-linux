...
module Make (Engine: Engine_Subset) = struct
  ...
  let check_overlap typ (lval, loc) (right_lval, right_loc) =
    if Ast_types.C.is_struct_or_union typ
    then
  ...
  let assign_lv_or_ret ~pos ~is_ret state lval expr =
    ...
    | `Value (valuation, lloc) ->
      ...
      let lval_copy =
        if is_ret || Ast_types.C.is_struct_or_union lval.typ || do_copy_at ~pos
        then find_lval expr
        else None
      in
  ...
  let filter_safe_arguments valuation call =
    ...
    let is_safe argument =
      ...
      && Ast_types.C.is_scalar argument.formal.vtype
    ...
    | Lval lv ->
      ...
      then
        Self.abort ~current:true
          ...
          Eva_ast.pp_exp expr;
      if determinate && Ast_types.C.is_scalar lv.typ
  ...
  let dump_state ~pos name state =
    Self.result ~dkey:Key.show ~pos ~stacktrace:true
      "%s:@\n@[<v>%a@]==END OF DUMP=="
  ...
  let pretty_arguments ~subdivnb state arguments =
    let is_scalar lval = Ast_types.C.is_scalar lval.Eva_ast.typ in
  ...
  let dump_state_file_exc ~pos ~subdivnb name arguments state =
    ...
    Self.feedback ~dkey:Key.show ~pos ~stacktrace:true
  ...
  let enter_scope kf variables state =
    ...
    let is_volatile varinfo =
      Ast_types.C.has_qualifier "volatile" varinfo.vtype
    in
