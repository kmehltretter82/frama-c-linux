...
let is_no_effect_frama_c_builtin (f:Eva_ast.lhost) =
  match f with
  | Var vi ->
...
let compute_transition_effect loop_effect = function
  ...
  | Call (None, f, _, _) when not (is_no_effect_frama_c_builtin f) ->
...
let is_integer lval = Ast_types.C.is_integral lval.Eva_ast.typ
...
module Make (Abstract: Engine_abstractions_sig.S) = struct
  ...
  let rec delta_assign context lval expr acc =
    ...
    let rec is_lval (e : Eva_ast.exp) = match e.node with
      ...
      | CastE (typ, e) -> Ast_types.C.is_integral typ && is_lval e
      | _ -> false
    in
    match Eva_ast.fold_to_integer expr with
    | Some i ->
    ...
    | None ->
      ...
      | CastE (typ, e) when Ast_types.C.is_integral typ ->
