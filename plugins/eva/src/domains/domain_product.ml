...
let product_category =
  Self.register_message_category ~group:Domain "domain_product" ~help:"inactive category"
...
= struct
  ...
  let lift_valuation side valuation =
    ...
    let fold f acc =
      valuation.Abstract_domain.fold
        ...
        acc
    in
    Abstract_domain.{ valuation with find; fold; }
  ...
  let show_expr =
    ...
    let show_expr_one_side category name show_expr = fun fmt exp ->
      if Self.is_message_category_enabled category
