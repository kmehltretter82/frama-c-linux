...
\begin{figure}[p]
\begin{center}
\includegraphics[width=\textwidth]{frama-c-gui-base.png}
\end{center}
\caption{The Frama-C GUI}
\label{fig:gui}
\end{figure}

\begin{figure}[p]
\begin{center}
\includegraphics[width=\textwidth]{frama-c-gui-wp-start.png}
\end{center}
\caption{Start \textsf{WP} from the GUI}
\label{wp-gui-view}
\end{figure}

\begin{figure}[p]
\begin{center}
\includegraphics[width=\textwidth]{frama-c-gui-wp-proof.png}
\end{center}
\caption{\textsf{WP} results and parameters}
\label{wp-gui-run}
\end{figure}

The WP view can be selected from the ``View \& Components'' sidebar
configuration (see ``WP view'' in figure~\ref{fig:gui}). The files and functions
can be selected in the ``Files'' view.

From there, one can start the proof of a property. Just select it in the
internal source view and choose \textsf{WP} from the contextual menu (see
figure~\ref{wp-gui-view}). After execution of the proof (see~\ref{wp-gui-run}),
the ``WP goals'' component contain the list of the goals that have been verified
(or not),  together with information about the solvers that have been used,
execution time, etc. The computation can also be run for a bundle of
properties if the contextual menu is open from a function or behavior
selection.

From the WP sidebar (see selector in figure~\ref{wp-gui-run}), one can finely
tune WP parameters, typically, select provers, time limits, choose the runtime
errors that should be detected, etc. Please refer to section~\ref{wp-cmdline}
for more details. It can also be used to run proofs on all functions of the
project. The help button gives access to the documentation integrated to the GUI.
...
\begin{center}
  \begin{tabular}{cl}
    ...
    \loadicon{feedback/doomed.png} & The property cannot be satisfied. \\
    \hline
  \end{tabular}
\end{center}
...
generated proof obligations and their status for each prover. By selecting a
goal, one can see the dependencies of the property, double-clicking on a goal
gives access to formula that corresponds to the goal.
...
Some tactics require one or several terms to be selected.
This feature has been lost when migrating from the old GTK based GUI to the new GUI.
If you are interested in funding this (last missing) feature of the old GUI, please contact us.
...
128. The tactic uses this test to tighten the bounds of $n$.
...
where $\sigma$ is the following substitution:
\[ \sigma \equiv
...
$m+1$ for bitmask) is a positive power of 2, the tactic only generates the guard
...
This is a variant of the \texttt{Lemma} tactic dedicated to \texttt{Havoc}
predicate generate by complex assigns clause. The user select an address, and
if the address is not assigned by the \texttt{Havoc} clause, the memory at this
...
\begin{description}
\item[\tt -wp-(no)-qed] enable Qed simplifications (default is: \texttt{yes}).
\item[\tt -wp-(no)-subst] extract and propagate equalities from the proof
  context (default is: \texttt{yes}).
...
\item[\tt -wp-(no)-clean] removes unused terms and variables from
  proof obligations (only available when qed simplifications are globally
  disabled) (default is: \texttt{yes}).
...
\item[\tt -wp-script <mode>] selects how to manage proof strategies and proof
  ...
  \end{itemize}
  This parameter can be customized using the \verb|FRAMAC_WP_SCRIPT| environment
  variable. If both the variable and the option are positioned, the option has
  the priority.
...
\item[\tt -wp-list-provers] lists all available provers.
  The option prints all the available provers from \textsf{Why3} configuration and from
  the current \texttt{PATH} with their respective version and \verb+-wp-prover <p>+
  code name. See \verb+-wp-why3-config+ and \verb+-wp-why3-detect+ for
  details.
...
\item[\tt -wp-why3-config=<file>] use the specified \textsf{Why3} configuration
  file for configuring provers. By default, uses \verb+'$HOME/.why3.conf'+
  if it exists.
\item[\tt -wp-(no)-why3-detect] collect all available provers from \verb+$PATH+
  in addition to the specified or default \textsf{Why3} configuration
  (set by default).
...
link declared \textsf{ACSL} symbols to external or predefined
...
Declared logic types, constructors, logic functions and logic predicates can be
linked to external Why3 library symbols, or aliased to other existing ACSL
builtins or declared symbols.

\newcommand{\ccc}{\texttt{,}\ldots\texttt{,}}
\newcommand{\user}{\texttt{"why3-symbol"}}
Each driver file contains a list of bindings with the following syntax:
\begin{center}
  \begin{tabular}{|rlll}
  \rule{0em}{1.2em}
  \texttt{type} & \textit{symbol} & \verb'='  \verb';' \\
  ...
  \texttt{logic} & \textit{type} \textit{symbol}
                   \verb'(' \textit{type}\ccc\textit{type} \verb')'
                 & \verb'=' \textit{property-tags} \user \verb';' \\
  \end{tabular}
\end{center}

It is also possible to define \emph{aliases} with other ACSL symbols, rather
than external links. In this case, you may replace \verb+=+ by
\verb+:=+ and use an ACSL identifier in place of the external \user.
No property tag is allowed when defining aliases.

\textsf{C}-Comments are allowed in the file. For overloaded \textsf{ACSL}
symbols, it is necessary to provide one \user symbol for each existing
signature. See also the default \textsf{WP} driver file for examples of external
links, in file \verb+wp.driver+ of the shared library.

Optional \textit{property-tags} can be given to
\texttt{logic} \user symbols to allow the \textsf{WP} plugin to perform
...
by an ACSL symbol. The available tags are depicted on figure~\ref{wp-driver-tags}.
...
\texttt{absorbent} \textit{e}\texttt{:} & specify the ACSL absorbent element of the operator: \\
                     & $e \odot x = x \odot e = e$ \\
\texttt{neutral} \textit{e}\texttt{:} & specify the ACSL neutral element of the operator: \\
                     &  $e \odot x = x \odot e = x$ \\
...
The \textsf{WP} plugin can use a session directory to store information to be used from one execution to another one.
...
\item[\tt -wp-cache <mode>] selects the cache mode to use with \textsf{Why-3}
  ...
  available, and \verb+none+ otherwise. The environment variable
  \verb|FRAMAC_WP_CACHE| can be used for the same purpose, if both the
  environment variable and the option are configured, the option is prioritary.
...
\item[\tt -wp-cache-dir <dir>] select the directory where cached results are
  ...
  directory. The environment variable \verb|FRAMAC_WP_CACHEDIR| can be used to
  customize the directory, if both the environment variable and the option
  are configured, the option is prioritary.
\end{description}
...
\item [\tt -wp-cache rebuild]: force prover execution and store all results in the cache. Previous results will be replaced with new ones, but entries for non relevant proofs would be kept and you might need a cleanup stage after. This mode is useful when you modify your \textsf{Why-3} or prover installation and you don't want to reuse your previous cache entries.
...
To activate proof strategies and search for new scripts for unproven goals, user
...
allows for searching in the proof obligation a \emph{pattern} that resembles the
...
following an heuristics designed to increase the stability for non-regression testing.
