...
(* utility function to turn on constant folding. *)
...
let mk_int_exp value =
  Cil.new_exp ~loc:Aorai_option.gen_loc
...
let rec term_to_exp t res =
  ...
  | TConst (LReal l_real) ->
    ...
    let Parsed (format, _) = Result.value_or_else ~error res  in
...
let get_bhv_aux_fct kf bhv =
  ...
  | None ->
    ...
    let ret_typ = Ast_types.C.add_ghost Cil_const.intType in
    let post_cond =
      [Normal,
       Logic_const.(
         new_predicate
...
let crosscond_to_exp generated_kf curr_f curr_status loc cond res =
  ...
  let rec expnode_convert =
    ...
    | TRel(rel,t1,t2) ->
      ...
      make_binop ~loc
        (rel_convert rel) (term_to_exp t1 res) (term_to_exp t2 res)
  in
...
let mk_gvar ?init ~ty name =
  ...
  let vi =
    try
      let ty' = Ast_types.C.add_attributes [("ghost", [])] ty in
    ...
    List.map
      (fun (e,i) ->
         { eiorig_name = e;
           einame = e;
           eival = mk_integer i;
           eiloc = Aorai_option.gen_loc;
           eihost = einfo})
      elements_l
  in
  einfo.eitems <- l;
  set_usedinfo name einfo;
  add_global (GEnumTag(einfo, Aorai_option.gen_loc));
  einfo
...
let is_state_exp state loc =
  ...
  then
    ...
    make_binop ~loc Eq
      (Cil.evar (Data_for_aorai.get_state_var state)) (Cil.one ~loc)
...
let mk_deterministic_lemma () =
  ...
  let make_one_lemma state =
    ...
    let lemma =
      Dlemma (name, [label],[],prop,[],Aorai_option.gen_loc)
    in
     ...
     let annot =
       Daxiomatic
         ("Aorai_pebble_axiomatic",
          List.map
            (fun li -> Dfun_or_pred(li,Aorai_option.gen_loc)) l,
          [],
          Aorai_option.gen_loc)
     in
...
let mk_transitions_stmt generated_kf loc f st res trans =
  List.fold_right
    (fun trans
      ...
      in
      let cond = make_binop ~loc LAnd (is_state_exp trans.start loc) exp in
      (tr_stmts @ aux_stmts,
       tr_vars @ aux_vars,
       Cil_datatype.Varinfo.Set.union tr_funcs new_funcs,
       make_binop ~loc LOr exp_from_trans cond,
