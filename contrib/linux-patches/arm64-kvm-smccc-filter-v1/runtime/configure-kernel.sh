...
"$linux_source/scripts/config" --file "$kernel_build/.config" \
	--set-str LOCALVERSION -frama-smccc-runtime \
...
for symbol in VIRTUALIZATION KVM; do
	value=$("$linux_source/scripts/config" --file "$kernel_build/.config" \
		--state "$symbol")
	if [[ $value != y ]]; then
		printf 'required CONFIG_%s is %s, expected y\n' \
			"$symbol" "$value" >&2
		exit 1
	fi
done
