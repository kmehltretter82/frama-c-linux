                     ...
                     unsigned char *dstbuf, size_t dlen )
{
    ...
    hs = ( slen + 1 ) / 2;
    S1 = secret;
    S2 = secret + slen - hs; //@ assert secret <= S2 <= secret + slen / 2;
...
static int ssl_decrypt_buf( ssl_context *ssl )
{
	...
	//@ assert ssl->in_msglen > 0; // manual inspection (relational invariant) 
     ...
     */
    ssl->in_msglen -= ( ssl->maclen + padlen ); //@ assert 0 <= ssl->in_msglen <= SSL_MAX_CONTENT_LEN;
