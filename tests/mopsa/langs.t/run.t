...
We compile a module that is then loaded by Frama-C
  ...
  $ dune exec --cache=disabled -- frama-c -mopsa-db mopsa-db.json -mopsa-target a.out
...
We omit the 'linker input unused' message emitted by GCC/Clang.
  $ dune exec --cache=disabled -- frama-c -mopsa-db mopsa-db.json -mopsa-target a.out -kernel-warn-key mopsa-db:non-c-source=inactive 2>&1 | grep -v "linker.*input"
  [kernel] adding .cc to known file extensions
  [kernel] Parsing cpp.cc (external front-end)
