/* run.config
   ...
   EXECNOW: BIN control_journal.ml BIN control_journal_bis.ml (@frama-c@ -journal-enable -eva -deps -out @EVA_OPTIONS@ -main f -journal-name control_journal.ml && cp control_journal.ml control_journal_bis.ml) > /dev/null 2> /dev/null
  CMD: @frama-c@
