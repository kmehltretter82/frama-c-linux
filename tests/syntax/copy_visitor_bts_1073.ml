  ...
  method! vglob_aux g =
    match g with
      | GFun (f,loc) ->
        ...
        let orig = Visitor_behavior.get_original_varinfo self#behavior f.svar in
        Visitor_behavior.set_varinfo self#behavior orig v2;
        Visitor_behavior.set_orig_varinfo self#behavior v2 orig;
        Visitor_behavior.reset_fundec self#behavior;
