...
let empty = make LocInfo.empty Memory_zone.bottom
...
let add_init_state_input state loc node =
  ...
  | _ ->
    ...
    in
    let new_outputs = Memory_zone.link state.under_outputs loc in
    make new_loc_info new_outputs

let test_and_merge ~old new_ =
  if LocInfo.is_included new_.loc_info old.loc_info
  && Memory_zone.is_included old.under_outputs new_.under_outputs
  then (false, old)
  else
  ...
  if LocInfo.is_bottom old.loc_info then true, new_
    ...
    let new_outputs =
      Memory_zone.meet old.under_outputs new_.under_outputs
    in
    let new_state =
...
let get_loc_nodes_and_part state loc =
  let process z nodes acc =
    if Memory_zone.intersects z loc then
      let z =
        if Memory_zone.equal loc z
        then Some loc
        (* Be careful not ot put None here, because if we have n_1 : (s1 =
           ...
           state...  *)
        else Some (Memory_zone.narrow z loc) in
      let add n acc =
...
let get_loc_nodes state loc =
  P.debug ~dkey ~level:2 "get_loc_nodes %a@.            in %a@."
    Memory_zone.pretty loc pretty state ;
  if Memory_zone.equal loc Memory_zone.bottom
  then  [], None (* nothing to do *)
  else
    let nodes = get_loc_nodes_and_part state loc in
    let undef_zone = Memory_zone.diff loc state.under_outputs in
    P.debug ~dkey ~level:2 "get_loc_nodes -> undef = %a@."
      Memory_zone.pretty undef_zone;
    let undef_zone =
      if (Memory_zone.equal undef_zone Memory_zone.bottom) then None
