...
class callableFunctionsVisitor ~libc = object(self)
  ...
  method! vvrbl vi =
    if not (self#already_seen vi) then begin
      if Ast_types.is_function_type vi.vtype && Metrics_base.consider_function ~libc vi
      then begin
    ...
    while not (Stack.is_empty todo) do
      ...
      if not (self#already_seen vi) then
        begin
          ...
          if Ast_types.is_function_type vi.vtype then self#visit_function vi
...
class deadCallsVisitor fmt ~libc cov_metrics =
  ...
  object(self)
    ...
    method! vvrbl vi =
      if Ast_types.is_function_type vi.vtype then self#reached_fun vi;
      Cil.SkipChildren (* no children anyway *)
