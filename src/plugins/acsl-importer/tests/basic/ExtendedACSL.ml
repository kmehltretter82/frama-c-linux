...
let visit_bar vis ext =
  ...
  | Ext_id idx ->
    let l = Bar_table.find idx in
    let l' = List.map_no_copy (Cil.visitCilPredicate vis) l in
...
let type_block_foo ctxt loc (name, decls) =
  let type_foo d =
    match d.extended_node with
    | Ext_lexpr ({ext_name = "inner_block_foo"; _} as ext) ->
      Logic_const.new_acsl_extension ~plugin:ext.ext_plugin "inner_block_foo" loc false
        (Ext_preds
           (List.map
              (ctxt.type_predicate ctxt (Logic_typing.Lenv.empty())) ext.ext_content))
    | _ -> ctxt.error d.extended_loc "Unexpected extension in block_foo"
  in
  Ext_annot(name, List.map type_foo decls)

let type_inner_block_foo c loc l =
  match l with
  | [p] ->
    let p = c.type_predicate c (Logic_typing.Lenv.empty()) p in
    Ext_preds [p]
  | _ -> c.error loc "Expected only one predicate in inner_block_foo"

let () =
  Acsl_extension.register_behavior ~plugin:"test" "foo" type_foo false;
  Acsl_extension.register_behavior ~plugin:"test" "bar" ~visitor:visit_bar ~printer:print_bar type_bar  false;
  Acsl_extension.register_behavior ~plugin:"test" "bla" type_bla false;
  Acsl_extension.register_code_annot_next_both ~plugin:"test" "baz" type_baz false;
  Acsl_extension.register_code_annot_next_loop ~plugin:"test" "lfoo" type_foo false;
  Acsl_extension.register_code_annot ~plugin:"test" "ca_foo" type_foo false;
  Acsl_extension.register_code_annot_next_stmt ~plugin:"test" "ns_foo" type_foo false;
  Acsl_extension.register_global ~plugin:"test" "global_foo" type_foo false;
  Acsl_extension.register_global_block ~plugin:"test" "block_foo" type_block_foo false;
  Acsl_extension.register_global ~plugin:"test" "inner_block_foo" type_inner_block_foo false

let run () =
  Ast.compute ();
  let debug = Kernel.debug_atleast 1 in
  let my_file = Temp_files.file ~keep:debug ~prefix:"Extend" ~suffix:".i" () in
  Filesystem.with_formatter_exn my_file (fun fmt -> File.pretty_ast ~fmt ());
