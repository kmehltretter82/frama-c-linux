...
int get_next_state1(const program_trace& trace_event, int curr_state = 0)
{
	switch(curr_state)
	{
		case 0: if(trace_event.bandwidth > 100000)
					return 1;
				 else
					return 0;
...
int get_next_state2(const program_trace& trace_event, int curr_state = 0)
{
	switch(curr_state)
	{
		case 0: if(trace_event.bandwidth > 10000)
					return 1;
				 else
					return 0;

		case 1: if(trace_event.bandwidth <= 10000)
					return 0;
				 else
					return 1;
	}
...
int main(int argc, char *argv[])
{
	...
	for(int i = 0 ; i < replicates; i++)
	{
		...
		{
			L.setline(line);
			string ip = L.get_field(2);
			float bandwidth = atof(L.get_field(12).c_str())/atof(L.get_field(10).c_str());
