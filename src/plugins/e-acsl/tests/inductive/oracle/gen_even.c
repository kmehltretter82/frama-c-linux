/* Generated by Frama-C */
#include "pthread.h"
#include "sched.h"
#include "signal.h"
#include "stddef.h"
#include "stdint.h"
#include "stdio.h"
#include "time.h"
extern  __attribute__((__FC_BUILTIN__)) int __e_acsl_sound_verdict;

/*@
inductive even(integer x) {
  case zero: \forall integer a; even(0);
  case pos: \forall integer a; a >= 2 ==> even(a - 2) ==> even(a);
  case neg: \forall integer a; a <= -2 ==> even(a + 2) ==> even(a);
  }
 */
/*@
predicate even(integer x) =
  x == 0 || (x >= 2 && even(x - 2)) || (x <= -2 && even(x + 2));

*/
int __gen_e_acsl_even(__e_acsl_mpz_struct * x);

int main(void)
{
  int __retres;
  __e_acsl_memory_init((int *)0,(char ***)0,8UL);
  {
    __e_acsl_mpz_t __gen_e_acsl_;
    int __gen_e_acsl_even_6;
    __e_acsl_assert_data_t __gen_e_acsl_assert_data = {.values = (void *)0};
    __gmpz_init_set_str(__gen_e_acsl_,"2",10);
    __gen_e_acsl_even_6 = __gen_e_acsl_even((__e_acsl_mpz_struct *)__gen_e_acsl_);
    __e_acsl_assert_register_int(& __gen_e_acsl_assert_data,"even(2)",0,
                                 __gen_e_acsl_even_6);
    __gen_e_acsl_assert_data.blocking = 1;
    __gen_e_acsl_assert_data.kind = "Assertion";
    __gen_e_acsl_assert_data.pred_txt = "even(2)";
    __gen_e_acsl_assert_data.file = "even.c";
    __gen_e_acsl_assert_data.fct = "main";
    __gen_e_acsl_assert_data.line = 18;
    __e_acsl_assert(__gen_e_acsl_even_6,& __gen_e_acsl_assert_data);
    __e_acsl_assert_clean(& __gen_e_acsl_assert_data);
    __gmpz_clear(__gen_e_acsl_);
  }
  /*@ assert even(2); */ ;
  {
    __e_acsl_mpz_t __gen_e_acsl__7;
    int __gen_e_acsl_even_8;
    __e_acsl_assert_data_t __gen_e_acsl_assert_data_2 =
      {.values = (void *)0};
    __gmpz_init_set_str(__gen_e_acsl__7,"3",10);
    __gen_e_acsl_even_8 = __gen_e_acsl_even((__e_acsl_mpz_struct *)__gen_e_acsl__7);
    __e_acsl_assert_register_int(& __gen_e_acsl_assert_data_2,"even(3)",0,
                                 __gen_e_acsl_even_8);
    __gen_e_acsl_assert_data_2.blocking = 1;
    __gen_e_acsl_assert_data_2.kind = "Assertion";
    __gen_e_acsl_assert_data_2.pred_txt = "!even(3)";
    __gen_e_acsl_assert_data_2.file = "even.c";
    __gen_e_acsl_assert_data_2.fct = "main";
    __gen_e_acsl_assert_data_2.line = 19;
    __e_acsl_assert(! __gen_e_acsl_even_8,& __gen_e_acsl_assert_data_2);
    __e_acsl_assert_clean(& __gen_e_acsl_assert_data_2);
    __gmpz_clear(__gen_e_acsl__7);
  }
  /*@ assert !even(3); */ ;
  {
    __e_acsl_mpz_t __gen_e_acsl__8;
    __e_acsl_mpz_t __gen_e_acsl_neg;
    int __gen_e_acsl_even_10;
    __e_acsl_assert_data_t __gen_e_acsl_assert_data_3 =
      {.values = (void *)0};
    __gmpz_init_set_si(__gen_e_acsl__8,4L);
    __gmpz_init(__gen_e_acsl_neg);
    __gmpz_neg(__gen_e_acsl_neg,
               (__e_acsl_mpz_struct const *)(__gen_e_acsl__8));
    __gen_e_acsl_even_10 = __gen_e_acsl_even((__e_acsl_mpz_struct *)__gen_e_acsl_neg);
    __e_acsl_assert_register_int(& __gen_e_acsl_assert_data_3,"even(-4)",0,
                                 __gen_e_acsl_even_10);
    __gen_e_acsl_assert_data_3.blocking = 1;
    __gen_e_acsl_assert_data_3.kind = "Assertion";
    __gen_e_acsl_assert_data_3.pred_txt = "even(-4)";
    __gen_e_acsl_assert_data_3.file = "even.c";
    __gen_e_acsl_assert_data_3.fct = "main";
    __gen_e_acsl_assert_data_3.line = 20;
    __e_acsl_assert(__gen_e_acsl_even_10,& __gen_e_acsl_assert_data_3);
    __e_acsl_assert_clean(& __gen_e_acsl_assert_data_3);
    __gmpz_clear(__gen_e_acsl__8);
    __gmpz_clear(__gen_e_acsl_neg);
  }
  /*@ assert even(-4); */ ;
  {
    __e_acsl_mpz_t __gen_e_acsl__9;
    __e_acsl_mpz_t __gen_e_acsl_neg_2;
    int __gen_e_acsl_even_12;
    __e_acsl_assert_data_t __gen_e_acsl_assert_data_4 =
      {.values = (void *)0};
    __gmpz_init_set_si(__gen_e_acsl__9,3L);
    __gmpz_init(__gen_e_acsl_neg_2);
    __gmpz_neg(__gen_e_acsl_neg_2,
               (__e_acsl_mpz_struct const *)(__gen_e_acsl__9));
    __gen_e_acsl_even_12 = __gen_e_acsl_even((__e_acsl_mpz_struct *)__gen_e_acsl_neg_2);
    __e_acsl_assert_register_int(& __gen_e_acsl_assert_data_4,"even(-3)",0,
                                 __gen_e_acsl_even_12);
    __gen_e_acsl_assert_data_4.blocking = 1;
    __gen_e_acsl_assert_data_4.kind = "Assertion";
    __gen_e_acsl_assert_data_4.pred_txt = "!even(-3)";
    __gen_e_acsl_assert_data_4.file = "even.c";
    __gen_e_acsl_assert_data_4.fct = "main";
    __gen_e_acsl_assert_data_4.line = 21;
    __e_acsl_assert(! __gen_e_acsl_even_12,& __gen_e_acsl_assert_data_4);
    __e_acsl_assert_clean(& __gen_e_acsl_assert_data_4);
    __gmpz_clear(__gen_e_acsl__9);
    __gmpz_clear(__gen_e_acsl_neg_2);
  }
  /*@ assert !even(-3); */ ;
  __retres = 0;
  __e_acsl_memory_clean();
  return __retres;
}

/*@ assigns \result;
    assigns \result \from *((__e_acsl_mpz_struct *)x); */
int __gen_e_acsl_even(__e_acsl_mpz_struct * x)
{
  __e_acsl_mpz_t __gen_e_acsl__2;
  int __gen_e_acsl_eq;
  int __gen_e_acsl_or_2;
  __gmpz_init_set_si(__gen_e_acsl__2,0L);
  __gen_e_acsl_eq = __gmpz_cmp((__e_acsl_mpz_struct const *)(x),
                               (__e_acsl_mpz_struct const *)(__gen_e_acsl__2));
  if (__gen_e_acsl_eq == 0) __gen_e_acsl_or_2 = 1;
  else {
    __e_acsl_mpz_t __gen_e_acsl__3;
    int __gen_e_acsl_ge;
    int __gen_e_acsl_and;
    int __gen_e_acsl_or;
    __gmpz_init_set_si(__gen_e_acsl__3,2L);
    __gen_e_acsl_ge = __gmpz_cmp((__e_acsl_mpz_struct const *)(x),
                                 (__e_acsl_mpz_struct const *)(__gen_e_acsl__3));
    if (__gen_e_acsl_ge >= 0) {
      __e_acsl_mpz_t __gen_e_acsl__4;
      __e_acsl_mpz_t __gen_e_acsl_sub;
      int __gen_e_acsl_even_3;
      __gmpz_init_set_si(__gen_e_acsl__4,2L);
      __gmpz_init(__gen_e_acsl_sub);
      __gmpz_sub(__gen_e_acsl_sub,(__e_acsl_mpz_struct const *)(x),
                 (__e_acsl_mpz_struct const *)(__gen_e_acsl__4));
      __gen_e_acsl_even_3 = __gen_e_acsl_even((__e_acsl_mpz_struct *)__gen_e_acsl_sub);
      __gen_e_acsl_and = __gen_e_acsl_even_3;
      __gmpz_clear(__gen_e_acsl__4);
      __gmpz_clear(__gen_e_acsl_sub);
    }
    else __gen_e_acsl_and = 0;
    if (__gen_e_acsl_and) __gen_e_acsl_or = 1;
    else {
      __e_acsl_mpz_t __gen_e_acsl__5;
      int __gen_e_acsl_le;
      int __gen_e_acsl_and_2;
      __gmpz_init_set_si(__gen_e_acsl__5,(long)(-2));
      __gen_e_acsl_le = __gmpz_cmp((__e_acsl_mpz_struct const *)(x),
                                   (__e_acsl_mpz_struct const *)(__gen_e_acsl__5));
      if (__gen_e_acsl_le <= 0) {
        __e_acsl_mpz_t __gen_e_acsl__6;
        __e_acsl_mpz_t __gen_e_acsl_add;
        int __gen_e_acsl_even_5;
        __gmpz_init_set_si(__gen_e_acsl__6,2L);
        __gmpz_init(__gen_e_acsl_add);
        __gmpz_add(__gen_e_acsl_add,(__e_acsl_mpz_struct const *)(x),
                   (__e_acsl_mpz_struct const *)(__gen_e_acsl__6));
        __gen_e_acsl_even_5 = __gen_e_acsl_even((__e_acsl_mpz_struct *)__gen_e_acsl_add);
        __gen_e_acsl_and_2 = __gen_e_acsl_even_5;
        __gmpz_clear(__gen_e_acsl__6);
        __gmpz_clear(__gen_e_acsl_add);
      }
      else __gen_e_acsl_and_2 = 0;
      __gen_e_acsl_or = __gen_e_acsl_and_2;
      __gmpz_clear(__gen_e_acsl__5);
    }
    __gen_e_acsl_or_2 = __gen_e_acsl_or;
    __gmpz_clear(__gen_e_acsl__3);
  }
  __gmpz_clear(__gen_e_acsl__2);
  return __gen_e_acsl_or_2;
}


