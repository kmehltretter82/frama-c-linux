/* Generated by Frama-C */
/*@ terminates \false;
    ensures \false;
    assigns \nothing;  */
extern void exit(int status);
/*@ assigns \nothing;  */
extern int printf(char const * , ...);
void e_acsl_assert(int predicate, char *kind, char *pred_txt, int line)
{
  if (predicate) {
    printf("%s failed at line %d.\nThe failing predicate is:\n%s.\n",kind,
           line,pred_txt);
    exit(1);
  }
  return;
}

int main(void)
{
  int __retres;
  int x;
  int y;
  x = -3;
  y = 2;
  /*@ assert -3 ≡ x; */ ;
  e_acsl_assert(! (-3 == x),(char *)"Assertion",(char *)"(-3 == x)",11);
  /*@ assert x ≡ -3; */ ;
  e_acsl_assert(! (x == -3),(char *)"Assertion",(char *)"(x == -3)",12);
  /*@ assert 0 ≢ ~0; */ ;
  e_acsl_assert(! (0 != ~ 0),(char *)"Assertion",(char *)"(0 != ~0)",13);
  /*@ assert x+1 ≡ -2; */ ;
  e_acsl_assert(! ((long long)x + (long long)1 == (long long)-2),
                (char *)"Assertion",(char *)"(x+1 == -2)",15);
  /*@ assert x-1 ≡ -4; */ ;
  e_acsl_assert(! ((long long)x - (long long)1 == (long long)-4),
                (char *)"Assertion",(char *)"(x-1 == -4)",16);
  /*@ assert x*3 ≡ -9; */ ;
  e_acsl_assert(! ((long long)x * (long long)3 == (long long)-9),
                (char *)"Assertion",(char *)"(x*3 == -9)",17);
  /*@ assert x/3 ≡ -1; */ ;
  {
    int e_acsl_1;
    /*@ assert 3 ≢ 0; */ ;
    e_acsl_assert(3 == 0,(char *)"Assertion",(char *)"(3 == 0)",18);
    e_acsl_1 = x / 3;
    e_acsl_assert(! (e_acsl_1 == -1),(char *)"Assertion",
                  (char *)"(x/3 == -1)",18);
  }
  
  /*@ assert 0xfffffffffff/0xfffffffffff ≡ 1; */ ;
  {
    int e_acsl_2;
    /*@ assert 0xfffffffffff ≢ 0; */ ;
    e_acsl_assert(0xfffffffffff == (long long)0,(char *)"Assertion",
                  (char *)"(0xfffffffffff == 0)",19);
    e_acsl_2 = (int)(0xfffffffffff / 0xfffffffffff);
    e_acsl_assert(! (e_acsl_2 == 1),(char *)"Assertion",
                  (char *)"(0xfffffffffff/0xfffffffffff == 1)",19);
  }
  
  /*@ assert x%2 ≡ -1; */ ;
  {
    int e_acsl_3;
    /*@ assert 2 ≢ 0; */ ;
    e_acsl_assert(2 == 0,(char *)"Assertion",(char *)"(2 == 0)",20);
    e_acsl_3 = x % 2;
    e_acsl_assert(! (e_acsl_3 == -1),(char *)"Assertion",
                  (char *)"(x%2 == -1)",20);
  }
  
  /*@ assert -3%-2 ≡ -1; */ ;
  {
    int e_acsl_4;
    /*@ assert -2 ≢ 0; */ ;
    e_acsl_assert(-2 == 0,(char *)"Assertion",(char *)"(-2 == 0)",21);
    e_acsl_4 = -3 % -2;
    e_acsl_assert(! (e_acsl_4 == -1),(char *)"Assertion",
                  (char *)"(-3%-2 == -1)",21);
  }
  
  /*@ assert 3%-2 ≡ 1; */ ;
  {
    int e_acsl_5;
    /*@ assert -2 ≢ 0; */ ;
    e_acsl_assert(-2 == 0,(char *)"Assertion",(char *)"(-2 == 0)",22);
    e_acsl_5 = 3 % -2;
    e_acsl_assert(! (e_acsl_5 == 1),(char *)"Assertion",
                  (char *)"(3%-2 == 1)",22);
  }
  
  /*@ assert ((x*2+(3+y))-4)+(x-y) ≡ -10; */ ;
  e_acsl_assert(! ((((long long)x * (long long)2 + ((long long)3 + (long long)y)) - (long long)4) + (
                   (long long)x - (long long)y) == (long long)-10),
                (char *)"Assertion",(char *)"(((x*2+(3+y))-4)+(x-y) == -10)",
                24);
  /*@ assert (0≡1) ≡ !(0≡0); */ ;
  e_acsl_assert(! ((0 == 1) == ! (0 == 0)),(char *)"Assertion",
                (char *)"((0==1) == !(0==0))",26);
  /*@ assert (0≤-1) ≡ (0>0); */ ;
  e_acsl_assert(! ((0 <= -1) == (0 > 0)),(char *)"Assertion",
                (char *)"((0<=-1) == (0>0))",27);
  /*@ assert (0≥-1) ≡ (0≤0); */ ;
  e_acsl_assert(! ((0 >= -1) == (0 <= 0)),(char *)"Assertion",
                (char *)"((0>=-1) == (0<=0))",28);
  /*@ assert (0≢1) ≡ !(0≢0); */ ;
  e_acsl_assert(! ((0 != 1) == ! (0 != 0)),(char *)"Assertion",
                (char *)"((0!=1) == !(0!=0))",29);
  /*@ assert (0≢0) ≡ !(1≢0); */ ;
  e_acsl_assert(! ((0 != 0) == ! (1 != 0)),(char *)"Assertion",
                (char *)"((0!=0) == !(1!=0))",31);
  __retres = 0;
  return (__retres);
}


