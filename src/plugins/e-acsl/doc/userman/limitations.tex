...
[kernel] warning: no entry point specified:
  you must call function `__e_acsl_memory_init' and `__e_acsl_memeory_clean'
  by yourself.
...
While it possible to add such intrumentation manually we recommend using
...
Programs programs providing custom definitions of \T{syscall}, \T{mmap} or
...
on these functions in order to track heap memory. Further, for heap memory
monitoring to function correctly input programs should be limited to allocating
and deallocating heap memory with POSIX-compliant memory management functions
