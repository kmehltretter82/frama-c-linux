...
let inject_in_instr env kf stmt = function
  ...
  | Call(result, caller, args, loc) ->
    ...
    let result, env =
      match caller with
      | Var cvi, _ when Libc.is_writing_memory cvi ->
...
and inject_in_block (env: Env.t) kf blk =
  blk.battrs <- Ast_attributes.(drop frama_c_ghost_else blk.battrs);
  let stmts, env =
    List.fold_left
      (fun (stmts, env) stmt ->
         let stmt, env = inject_in_stmt env kf stmt in
...
let inject_in_fundec main fundec =
  ...
  let unghost_formal vi =
    unghost_local vi ;
    vi.vattr <- Ast_attributes.(drop frama_c_ghost_formal vi.vattr)
  in
...
let unghost_vi vi =
  ...
  | { tnode = TFun (res, Some l, va); tattr } ->
    ...
    let retype (n, t, a) =
      n, t, Ast_attributes.(drop frama_c_ghost_formal a)
    in
      ...
      let mk_fct_call vi =
        let stmt =
          Cil.mkStmtOneInstr ~valid_sid:true
            (Call(None, Cil.var vi, [], Location.unknown))
        in
        vi.vreferenced <- true;
        stmt
      in
