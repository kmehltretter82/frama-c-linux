...
let copy ?(smart = true) t =
  if smart && Options.O.get () > 0 then
    ...
    | TSizeOf typ -> tsizeof ~smart ~loc:t.term_loc typ
    | TAlignOf typ -> talignof ~smart ~loc:t.term_loc typ
...
let trange_array ?(smart = true) ?(loc = Fileloc.unknown) t  =
  ...
  let approx typ =
    copy ~smart @@
    tbinop ~smart ~loc Div
      (tbinop ~smart MinusA (tblock_length t) (toffset t))
      (tsizeof ~smart typ)
  in
  let rec sizes typ =
        ...
        let range size =
          Logic_const.trange ~loc
            (Option.some @@ Logic_const.tint ~loc Z.zero,
             Option.some @@
             tbinop ~smart ~loc MinusA size (Logic_const.tint ~loc Z.one))
        in
        List.fold_left
          (fun lval size ->
