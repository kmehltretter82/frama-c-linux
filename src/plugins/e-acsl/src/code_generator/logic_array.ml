        ...
        (fun v _ -> [
             Smart_stmt.assigns
               ...
               (Cil.mkBinOp_exn
                  ~loc
                  Div
                  (Cil.mkBinOp_exn ~loc MinusA block_length_exp offset_exp)
                  sizeof_exp)
           ])
    in
    len_exp, env
...
let comparison_to_exp ~loc kf env ~name bop array1 array2 =
  ...
  *)
  ...
  let cond = Cil.mkBinOp_exn ~loc Ne array1_iter_e array2_iter_e in
  ...
  let prepend_coercion_check ~name env stmts array len =
    ...
    if array_orig == array then
      ...
      in
      let e = Cil.mkBinOp_exn ~loc Le len len_orig in
      let p =
        Logic_const.prel
          ~loc
  ...
  let stmt =
    Smart_stmt.if_stmt
      ~loc
      ~cond:(Cil.mkBinOp_exn ~loc Eq len1_exp len2_exp)
      then_blk
