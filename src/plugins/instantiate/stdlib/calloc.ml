...
let pinitialized_len ?loc alloc_type num size =
  ...
  let initialized ?loc t =
    let t = match t.term_node, Ast_types.Acsl.unroll_logic t.term_type with
...
let well_typed_call ret _fct args =
  ...
  | Some ret, [ _ ; _ ] ->
    ...
    Cil.isCompleteType (Ast_types.C.direct_pointed_type t)
  | _ -> false
...
let key_from_call ret _fct _ =
  ...
  | Some ret ->
    ...
    Ast_types.C.direct_pointed_type ret_t
