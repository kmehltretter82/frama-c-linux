  ...
  let return_deps,deps =
    ...
    | Writes assigns ->
      ...
      let return_assigns =
        ...
        | [] when Ast_types.C.is_void rt_typ ->
  ...
  module Computer = struct
    ...
    let transfer_instr stmt (i: instr) (state: t) =
      ...
      | Local_init(v, AssignInit i, _) ->
        let rec aux lv i acc =
          ...
          | CompoundInit (ct, initl) ->
            ...
            let implicit =
              not (Ast_types.C.is_array ct &&
                   (Ast_types.C.(is_scalar (direct_array_element ct))
      ...
      | Call (lvaloption,funclv,argl,loc) ->
        transfer_call stmt lvaloption funclv argl loc state
    ...
    let transfer_guard s e d =
      ...
      let t1 = Ast_types.C.unroll (Cil.typeOf e) in
      let do_then, do_else =
        if Ast_types.C.is_integral t1 || Ast_types.C.is_ptr t1
