    ...
    let add_block_length b acc =
      let bl =
        ...
        let convert start_bits end_bits =
          let congr_succ i = Integer.(equal zero (erem (succ i) eight)) in
          let congr_or_zero i = Integer.(equal zero i || congr_succ i) in
          assert (congr_or_zero start_bits || congr_or_zero end_bits);
          let start_bytes = Integer.(ediv (Integer.succ start_bits) eight) in
          let end_bytes =   Integer.(ediv (Integer.succ end_bits)   eight) in
