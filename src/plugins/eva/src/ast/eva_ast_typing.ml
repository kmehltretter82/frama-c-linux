...
let rec type_of_offset (basetyp : typ) : offset -> typ = function
  | NoOffset -> basetyp
  | Index (_, o) ->
    type_of_offset (Ast_types.C.direct_array_element basetyp) o
  | Field (fi, o) ->
    let base_attrs = (Ast_types.C.unroll basetyp).tattr in
    ...
    let base_attrs =
      if Ast_attributes.(contains frama_c_mutable fi.fattr) then
        ...
        base_attrs
    in
    type_of_offset (Ast_types.C.add_attributes base_attrs fi.ftype) o
...
let type_of_exp_node : exp_node -> typ = function
  ...
  | Lval lv -> Ast_types.C.remove_qualifiers lv.typ
  | UnOp (_, _, t) -> t
  | BinOp (_, _, _, t) -> t
  | CastE (t, _) -> t
  | AddrOf lv -> Cil_const.mk_tptr lv.typ
  | StartOf lv ->
    match Ast_types.C.unroll lv.typ with
