  ...
  then V.inject_float Fval.nan
    ...
    let fl, fu = match fstring with
      ...
      | Some string when fkind = Cil_types.FLongDouble || all_rounding () ->
        let Parsed p = Floating_point.parse string in
        ...
        if Floating_point.(is_infinite p.lower && is_infinite p.upper) then
          begin
        ...
        else Floating_point.(to_float p.lower, to_float p.upper)
      | None | Some _ -> f, f
    in
