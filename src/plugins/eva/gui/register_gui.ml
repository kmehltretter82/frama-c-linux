...
module Select (Eval: Eval) = struct
  ...
  let select_lv main_ui loc lv =
    if Ast_types.is_scalar (Cil.typeOfLval lv)
  ...
  let right_click_values_computed main_ui menu localizable =
    ...
    | PLval (_kfopt, ki, lv) ->
      ...
      begin
        (match lv with
         | Var _,NoOffset when Ast_types.is_fun ty ->
           () (* direct calls are handled by [Design]. *)
         | Mem _ as pf, NoOffset when Ast_types.is_fun ty -> begin
             (* Function pointers *)
             (* get the list of functions in the values *)
             let f = Eva_ast.translate_host pf in
             let state =
               match ki with
