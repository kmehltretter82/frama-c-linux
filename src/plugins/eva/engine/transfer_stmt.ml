...
module Make (Engine: Engine_sig.S) = struct
  ...
  let check_overlap typ (lval, loc) (right_lval, right_loc) =
    if Ast_types.is_struct_or_union_type typ
    then
  ...
  let assign_lv_or_ret ~is_ret state kinstr lval expr =
    ...
    | `Value (valuation, lloc) ->
      ...
      let lval_copy =
        if is_ret || Ast_types.is_struct_or_union_type lval.typ || do_copy_at kinstr
        then find_lval expr
        else None
      in
  ...
  let filter_safe_arguments valuation call =
    ...
    let is_safe argument =
      ...
      && Ast_types.is_scalar_type argument.formal.vtype
    ...
    | Lval lv ->
      ...
      then
        Self.abort ~current:true
          ...
          Eva_ast.pp_exp expr;
      if determinate && Ast_types.is_scalar_type lv.typ
  ...
  let pretty_arguments ~subdivnb state arguments =
    let is_scalar lval = Ast_types.is_scalar_type lval.Eva_ast.typ in
