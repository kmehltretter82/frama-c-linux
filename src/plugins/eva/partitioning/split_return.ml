...
module ReturnUsage = struct
  ...
  let add_call (uf: return_usage_per_fun) lv_opt e_fun =
    ...
    | Lval (Var vi, NoOffset), Some lv
      when Ast_types.is_integral_or_pointer_type (Cil.typeOfLval lv) ->
  ...
  let add_alias (uf: return_usage_per_fun) lv_dest e =
    ...
    | CastE (typ, { enode = Lval lve })
      when Ast_types.is_integral_or_pointer_type typ &&
           Ast_types.is_integral_or_pointer_type (Cil.typeOfLval lve)
      ->
  ...
  let add_compare_ct uf i lv =
    if Ast_types.is_integral_or_pointer_type (Cil.typeOfLval lv) then
      let u = find_or_default uf lv in
  ...
  let add_compare (uf: return_usage_per_fun) cond =
    ...
    (match cond.enode with
     ...
     | BinOp ((Eq | Ne), ct, {enode = CastE (typ, {enode = Lval lv})}, _) ->
       if Ast_types.is_integral_or_pointer_type typ &&
          Ast_types.is_integral_or_pointer_type (Cil.typeOfLval lv)
       then add ct lv
       else uf
  ...
  let add_null_pointers_split (ru: return_split): return_split =
    ...
    let aux kf acc =
      if Ast_types.is_pointer_type (Kernel_function.get_return_type kf) then
