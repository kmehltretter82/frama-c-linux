...
struct
  ...
  struct
    ...
    let typ_of_chunk x =
      ...
      | ByRef -> Ast_types.C.direct_pointed_type x.vtype
      | _ -> x.vtype
  ...
  let vtype m x =
    match m with
    | CVAL | HEAP -> x.vtype
    | CTXT _ | CREF -> Ast_types.C.direct_pointed_type x.vtype
    | CARR _ -> Cil_const.mk_tarray (Ast_types.C.direct_pointed_type x.vtype) None
