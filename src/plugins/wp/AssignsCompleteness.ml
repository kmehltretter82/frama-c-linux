...
let check_assigns kf assigns =
  ...
  let vassigns acc a =
    ...
    let acc =
      if Ast_types.C.is_ptr res_t && not (List.exists assigns_result froms) then
        incomplete acc
          begin fun kf ->
            Wp_parameters.warning ~wkey:wkey_pedantic
              ~once:true ~source:(Fileloc.loc_start (Kernel_function.get_location kf))
    ...
    let vfrom acc = function
      | t, FromAny when Ast_types.Acsl.is_ptr t.it_content.term_type ->
        incomplete acc
          begin fun _kf ->
            Wp_parameters.warning ~wkey:wkey_pedantic
              ~once:true ~source:(Fileloc.loc_start t.it_content.term_loc)
