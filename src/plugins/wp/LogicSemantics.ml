...
struct
  ...
  let rec cvsort_of_ltype src_ltype =
    match Logic_utils.unroll_logic_type ~unroll_typedef:false src_ltype with
    | Lboolean -> L_bool
    | Linteger -> L_integer
    | Lreal -> L_real
    | Ctype src_ctype ->
  ...
  let term_cast_to_real env t =
    let src_ltype = Logic_utils.unroll_logic_type ~unroll_typedef:false t.term_type in
  ...
  let rec term_cast_to_ltype env dst_ltype t =
    match Logic_utils.unroll_logic_type ~unroll_typedef:false dst_ltype with
