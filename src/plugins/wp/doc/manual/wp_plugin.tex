...
\item[\tt -wp-bool-range] optional support for \verb+_Bool+ range. This option assumes
  there is no \emph{trap representations}. Hence, integer values with boolean type are
  either equal to \verb+0+ or \verb+1+. Actually, the only way for forge other values
...
\item[\tt -wp-(no)-warn-separation] this option (de)activate the
  warnings about separation hypotheses
