...
let add_dpoints_to env tgt = function
  ...
  | None ->
    ...
    Options.warning ~wkey ~source:loc
...
let rec add_assigns_from env ~iscalled ~from tgt =
  ...
  | TUpdate _ | Ttypeof _ | Ttype _ | Tempty_set | Trange _ ->
    Options.warning ~source:tgt.term_loc
...
let add_bassigns ~called ~map ~kf ~ki ~bhv ~formals ~result = function
  ...
  | Writes ws as asgn ->
    ...
    | Some result ->
      ...
      then
        let source = Access.location context in
        Options.warning ~wkey ~source
...
let add_allocation ~map ~called ~kf ~ki ~bhv ~formals ~result alloc =
  ...
  | FreeAlloc _ ->
    ...
    let source = Kernel_function.get_location kf in
    Options.not_yet_implemented ~source
