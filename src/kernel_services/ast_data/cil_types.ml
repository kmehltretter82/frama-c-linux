...
and constant =
  ...
  (** Integer constant. Give the ikind (see ISO9899 6.1.3.2) and the
      ...
      Note that GCC does not allow 128-bit integer constants on architectures
      where long long is less than 128 bits. Therefore we do not consider
      supporting such constants. *)
  | CChr of char
...
and str_literal =
  ...
  *)
  ...
  (** Initialization of a const wide character string literal.
      ...
      {!Machine.wchar_kind}.  Such a constant has type array of
      {!Machine.wchar_type}. The escape characters in the string
...
and initinfo = { mutable init : init_or_str option }
