...
struct
  ...
  struct
    ...
    let rec pretty : type a. Format.formatter -> t -> unit =
  ...
  module rec Memory :
    Hptmap.V with type t = Obj.t memory (* FieldMap.t memory *) =
  struct
  ...
  and MemorySafe :
  sig
    type t = Obj.t (* FieldMap.t *) memory
    val pretty : Format.formatter -> t -> unit
    val hash : t -> int
    val equal : t -> t -> bool
    val compare : t -> t -> int
  ...
  struct
    module M = Prototype (FieldMap)
    type t = Obj.t memory
    let pretty fmt x = M.pretty fmt (Obj.magic x)
    let hash x = M.hash (Obj.magic x)
    let equal x y = M.equal (Obj.magic x) (Obj.magic y)
    let compare x y = M.compare (Obj.magic x) (Obj.magic y)
  end
  ...
  let raw m =
    let rec aux acc = function
      ...
      | Struct s ->
        ...
        FieldMap.fold (fun _ x acc -> aux acc (Obj.magic x)) s.struct_value acc
      | Union u ->
  ...
  let is_included : t -> t -> bool =
    let rec is_included (m1:t) (m2:t) = match m1, m2 with
      | _, Raw r -> Bit.is_included (raw (Obj.magic m1)) r
      | Scalar s1, Scalar s2 ->
      ...
      | Struct s1, Struct s2 ->
        ...
        FieldMap.binary_predicate Hptmap_sig.NoCache UniversalPredicate
          ...
          (Obj.magic s1.struct_value) (Obj.magic s2.struct_value)
      | Union u1, Union u2 ->
  ...
  let join f =
    let rec join m1 m2 =
      ...
      | Struct s1, Struct s2 when are_structs_compatible s1 s2 ->
        ...
        Struct {
          s1 with
          struct_value = Obj.magic struct_value;
          struct_padding = Bit.join s1.struct_padding s2.struct_padding;
        }
      | Struct s, Raw b | Raw b, Struct s ->
        Struct {
          s with
          struct_value = Obj.repr @@ FieldMap.map (fun s -> join (Raw b) s) (Obj.obj s.struct_value);
      ...
      | _,_ ->
        Raw (Bit.join (raw (Obj.magic m1)) (raw (Obj.magic m2)))
    in
    join
  ...
  let read (map : t -> 'a) (reduce : 'a -> 'a -> 'a) m offset : 'a =
    let rec aux m offset =
      ...
      | Field (fi, offset'), Struct s when s.struct_info.ckey = fi.fcomp.ckey ->
        begin try
            let m' = FieldMap.find fi (Obj.magic s.struct_value) in
            aux m' offset'
          with Not_found ->
      ...
      | _, _ ->  map (Raw (raw (Obj.magic m))) (* structure mismatch *)
    in
    aux m offset
    ...
    | Field (fi, offset') ->
      if fi.fcomp.cstruct then (* Structures *)
        let old = match m with
          ...
          | _ -> {
              struct_value = Obj.magic @@ FieldMap.empty;
              struct_info = fi.fcomp;
              struct_padding = raw (Obj.magic @@ m)
            }
        in
        let write' opt =
        ...
        in
        Struct {
          old with struct_value = Obj.magic (FieldMap.replace write' fi (Obj.magic old.struct_value))
        }
      else (* Unions *)
        let old = match m with
          | Union u when Field.equal u.union_field fi -> u
          | _ ->
            let b = raw (Obj.magic m) in {
              union_value = Raw b;
              union_field = fi;
              union_padding = b;
            }
  ...
  let rec weak_erase b = function
    ...
    | Struct s -> Struct {
        ...
        struct_value = Obj.magic @@ FieldMap.map (weak_erase b) (Obj.magic s.struct_value);
      }
    | Union u -> Union {
        u with
