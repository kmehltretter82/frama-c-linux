...
module V = struct
  ...
  let pretty_base_offsets_typ typ fmt b i =
    let typ_match = match Option.map Ast_types.unroll_node typ with
      | Some (TPtr typ_pointed) ->
        if Ast_types.is_void typ_pointed then None else Some typ_pointed
      | _ -> None
    in
    try
      ...
      | Some a -> (* Multiple offsets. We use a set notation *)
        ...
        let conv_offset' o =
          let o, ok = conv_offset o in
          if o = NoOffset then
            let o' = match Ast_types.unroll_node typ_base with
  ...
  let pretty_typ typ fmt v =
    ...
    | Map m ->
      try
        ...
        match Ival.project_small_set ival, Option.map Ast_types.unroll_node typ with
