...
and pp_varinfo fmt varinfo =
  if print_full_varinfo then
    Format.fprintf fmt
      ...
      (* the code below prevents infinite recursion *)
      (if varinfo.vlogic_var_assoc = None then "None" else "Some " ^ (Option.get varinfo.vlogic_var_assoc).lv_name)
  else
...
and pp_instr fmt = function
  ...
  | Call(lval_option,lv,exp_list,location) ->
    Format.fprintf fmt "Call(%a,%a,%a,%a)"  (pp_option pp_lval) lval_option  pp_lval lv
