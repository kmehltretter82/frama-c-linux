...
and constFoldBinOpToInt ~machdep bop e1 e2 =
  ...
  | Some i1, Some i2 -> begin
      ...
      | Div ->
        if Integer.(equal zero i2) && Integer.(is_zero (erem i1 i2)) then None
        else Some (Integer.ediv i1 i2)
      | Mod -> if Integer.(equal zero i2) then None else Some (Integer.erem i1 i2)
      ...
      | Cil_types.Le -> comp Integer.leq
      | Cil_types.Ge -> comp Integer.geq
