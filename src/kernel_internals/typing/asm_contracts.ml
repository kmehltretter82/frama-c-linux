...
let access_ptr_elts ~loc tlv =
  ...
  let base, basetype =
    if Ast_types.Acsl.is_logic_void_pointer basetype then begin
...
let extract_mem_term ~loc acc tlv =
  match Ast_types.Acsl.unroll_logic (Cil.typeOfTermLval tlv) with
    ...
    method! vinst i =
      ...
      | Asm(_, _, Some { asm_outputs; asm_inputs; asm_clobbers }, loc) ->
        ...
        let lv_from =
          List.filter
            (fun lv ->
               not (Ast_types.Acsl.is_logic_array (Cil.typeOfTermLval lv)))
            lv_from
        in
        ...
        let to_id_from lv =
          ...
          let term =
            if Ast_types.Acsl.is_logic_pointer typ ||
               Ast_types.Acsl.is_logic_array typ
        ...
        let mk_initialized x =
          ...
          if Ast_types.Acsl.is_logic_void typ then None
          else begin
