...
let gen_redefinable_macro fmt macro def =
  let redef_name = "__FC_FORCE_" ^ String.strip_underscores macro in
...
let gen_define_custom_macros fmt censored mach =
  ...
  let is_same_macro m1 m2 =
    String.strip_underscores m1 = String.strip_underscores m2
  in
...
let no_signedness s =
  let s = Option.value  ~default:s (String.strip_prefix "signed" s) in
  let s = Option.value  ~default:s (String.strip_prefix "unsigned" s) in
