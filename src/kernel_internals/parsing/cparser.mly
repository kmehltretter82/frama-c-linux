...
let make_expr start_end_pos expr_node =
  let expr_loc = Fileloc.of_lexing_loc start_end_pos in
  { expr_loc; expr_node }
...
let in_block sloc l =
  let loc = Fileloc.of_lexing_loc sloc in
  match l with
...
| f=IDENT LPAREN prms=old_parameter_list_ne RPAREN decls=old_pardef_list SEMICOLON {
    let dloc = Fileloc.of_lexing_loc $loc in
    let floc = Fileloc.of_lexing_loc $loc(f) in
...
| BUILTIN_VA_ARG LPAREN expression COMMA type_name RPAREN {
    ...
    let b, d = $5 in
    let loc = Fileloc.of_lexing_loc $loc($5) in
    let loc_f = Fileloc.of_lexing_loc $loc($1) in
...
| BUILTIN_TYPES_COMPAT LPAREN type_name COMMA type_name RPAREN {
    ...
    let b2,d2 = $5 in
    let loc_f = Fileloc.of_lexing_loc $loc($1) in
    let loc1 = Fileloc.of_lexing_loc $loc($3) in
    let loc2 = Fileloc.of_lexing_loc $loc($5) in
...
| string_list {
    let loc = Fileloc.of_lexing_loc $loc in
...
| id = id_or_typename_as_id COLON s = annotated_statement {
    let loc = Fileloc.of_lexing_loc $loc(id) in
    match s with
...
| spec=ioption(SPEC) specif=decl_spec_list decls=init_declarator_list? SEMICOLON {
    let loc = Fileloc.of_lexing_loc ($symbolstartpos,$endpos) in
...
| ENUM   just_attributes                LBRACE enum_list maybecomma RBRACE
                                                   { Tenum   ("", Some $4, $2), $1 }
| NAMED_TYPE      {
      (Tnamed $1, Fileloc.of_lexing_loc $sloc)
      }
...
| IDENT EQ expression { ($1, $3, Fileloc.of_lexing_loc $sloc) }
...
| id_or_typename { ($1, JUSTBASE, Fileloc.of_lexing_loc $sloc) }
...
| specif=decl_spec_list name=abstract_decl {
    let d, a = name in
    if a <> [] then begin
      let loc = Fileloc.of_lexing_loc ($loc(name)) in
...
| IDENT parameter_list_startscope rest_par_list RPAREN ghost_parameter_opt{
    let (params, isva) = $3 in
    let ghost = $5 in
    let floc = Fileloc.of_lexing_loc $loc($1) in
...
| ASM LPAREN string_constant RPAREN attributes {
    let loc = Fileloc.of_lexing_loc $loc($3) in
    ("__asm__",
...
| id_or_typename_as_id paren_attr_list_ne {
    let loc = Fileloc.of_lexing_loc $loc($1) in
...
| id_or_typename_as_id LPAREN RPAREN {
    let loc1 = Fileloc.of_lexing_loc $loc($1) in
    let loc2 =
...
| line=one_string rest=asmtemplate {
    let loc = Fileloc.of_lexing_loc $loc(line) in
