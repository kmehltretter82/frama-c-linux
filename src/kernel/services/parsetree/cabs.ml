...
and field_group =
  ...
  | STATIC_ASSERT_FG of expression * string * Fileloc.t
...
and name = string * decl_type * attribute list * Fileloc.t
...
and enum_item = string * expression * Fileloc.t
...
and definition =
  | FUNDEF of
      (Logic_ptree.spec*Fileloc.t) option * single_name * block *
      Fileloc.t * Fileloc.t
  | DECDEF of (Logic_ptree.spec*Fileloc.t) option * init_name_group * Fileloc.t
  (* global variable(s), or function prototype *)
  | TYPEDEF of name_group * Fileloc.t
  | ONLYTYPEDEF of specifier * Fileloc.t
  | GLOBASM of string * Fileloc.t
  | PRAGMA of expression * Fileloc.t
  | STATIC_ASSERT of expression * string * Fileloc.t
  | LINKAGE of string * Fileloc.t * definition list (* extern "C" { ... } *)
...
and raw_statement =
  | NOP of attribute option * Fileloc.t
  | COMPUTATION of expression * Fileloc.t
  | BLOCK of block * Fileloc.t * Fileloc.t
  | IF of expression * statement * statement * Fileloc.t
  | WHILE of loop_invariant * expression * statement * Fileloc.t
  | DOWHILE of loop_invariant * expression * statement * Fileloc.t
  | FOR of loop_invariant *
           for_clause * expression * expression * statement * Fileloc.t
  | BREAK of Fileloc.t
  | CONTINUE of Fileloc.t
  | RETURN of expression * Fileloc.t
  | SWITCH of expression * statement * Fileloc.t
  | CASE of expression * statement * Fileloc.t
  | CASERANGE of expression * expression * statement * Fileloc.t
  | DEFAULT of statement * Fileloc.t
  | LABEL of string * statement * Fileloc.t
  | GOTO of string * Fileloc.t
  | COMPGOTO of expression * Fileloc.t (* GCC's "goto *exp" *)
  ...
  *)
  ...
  (** [TRY_CATCH(s,clauses,loc)] catches exceptions thrown by execution of
      ...
      [single_name] is [None], all exceptions are caught by the corresponding
      clause.
  ...
  *)
  ...
  | CODE_ANNOT of (Logic_ptree.code_annot * Fileloc.t) (* annotations *)
  | CODE_SPEC of (Logic_ptree.spec * Fileloc.t) (* annotations *)
...
and expression = { expr_loc : Fileloc.t; expr_node: cabsexp }
