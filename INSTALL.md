...
6. Install *recommended* provers for Frama-C/WP:

The WP plug-in uses external theorem provers to finish its job. We recommend to
install at least one prover among Alt-Ergo, CVC4 or Z3, which are the most
efficient ones to discharge proof obligations generated by WP. All three are
available _via_ opam or homebrew, but don't forget to reconfigure `why3` after
installation (which is a required dependency of frama-c, installed at previous step).
